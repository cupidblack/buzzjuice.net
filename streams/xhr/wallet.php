            ...
            if ($_GET['type'] == 'pro') {
                ...
                if (!empty($_GET['pro_type']) && in_array($_GET['pro_type'], array_keys($wo["pro_packages"]))) {
                    ...
                    if ($wo["pro_packages"][$_GET['pro_type']]['price'] > $wo['user']['wallet']) {
                        ...
                        $currency_code = $wo['config']['currency'];
        
                        $create_payment_log = mysqli_query($sqlConnect, "INSERT INTO " . T_PAYMENT_TRANSACTIONS . " (`userid`, `kind`, `amount`, `currency_code`, `payment_status`, `notes`) VALUES ({$wo['user']['user_id']}, 'PRO', {$price}, '{$currency_code}', '{$payment_status}', '{$notes}')");
                        ...
                        // Update cache and return success response
                        cache($wo['user']['id'], 'users', 'delete');
                        $data['status'] = 200;
