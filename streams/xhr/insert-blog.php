        ...
        if (empty($error)) {
            ...
            if ($last_id && is_numeric($last_id)) {
                ...
                $fileInfo = array(
                    ...
                    'types' => 'jpeg,jpg,png,bmp,gif,webp', // Added 'webp'
                    'crop' => array(
                        'width' => 1200,
                        'height' => 600
                    )
                ...
                if (!empty($media) && is_array($media) && !empty($media['filename'])) {
                    ...
                    error_log("Failed to upload thumbnail file: " . json_encode($fileInfo));
                }
            }

				// Handle Audio Upload
                if (!empty($_FILES["blog_audio"]["tmp_name"])) {
                    $fileInfo = array(
                        'file'  => $_FILES["blog_audio"]["tmp_name"],
                        'name'  => $_FILES['blog_audio']['name'],
                        'size'  => $_FILES["blog_audio"]["size"],
                        'type'  => $_FILES["blog_audio"]["type"],
                        'types' => 'mp3,wav'
                    );
                    $audio = Wo_ShareFile($fileInfo);
                    if (!empty($audio) && !empty($audio['filename'])) { // Validate $audio
                        Wo_UpdateBlog($last_id, array(
                            "audio" => $audio['filename']
                        ));
                    } else {
                        error_log("Audio upload failed: " . print_r($fileInfo, true)); // Log the failure
                    }
                }
