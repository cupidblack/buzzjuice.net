...
if ($f == 'products') {
    ...
    if ($s == 'tracking') {
        if (!empty($_POST['tracking_url']) && !empty($_POST['tracking_id']) && !empty($_POST['order_hash']) && filter_var($_POST['tracking_url'], FILTER_VALIDATE_URL)) {
            ...
            $vendor_contact = !empty($_POST['vendor_contact']) ? Wo_Secure($_POST['vendor_contact']) : null;
            $delivery_agent = !empty($_POST['delivery_agent']) ? Wo_Secure($_POST['delivery_agent']) : null;
            $warranty_policy = !empty($_POST['warranty_policy']) ? Wo_Secure($_POST['warranty_policy']) : null;
            $currency = !empty($_POST['currency']) ? Wo_Secure($_POST['currency']) : null;
            $delivery_fee = !empty($_POST['delivery_fee']) && is_numeric($_POST['delivery_fee']) ? Wo_Secure($_POST['delivery_fee']) : null;
    
            // New fields for payment service
            $payment_service_name = !empty($_POST['payment_service_name']) ? Wo_Secure($_POST['payment_service_name']) : null;
            $payment_service_account = !empty($_POST['payment_service_account']) ? Wo_Secure($_POST['payment_service_account']) : null;
            $payment_service_username = !empty($_POST['payment_service_username']) ? Wo_Secure($_POST['payment_service_username']) : null;
    
            $order = $db->where('hash_id', $hash_id)->where('product_owner_id', $wo['user']['user_id'])->getOne(T_USER_ORDERS);
            if (!empty($order)) {
                // Update tracking details and set status to 'accepted'
                $db->where('hash_id', $hash_id)->update(T_USER_ORDERS, array(
                    ...
                    'payment_service_account' => $payment_service_account,
                    'payment_service_username' => $payment_service_username,
    ...
    if ($s == 'change_status') {
        if (!empty($_POST['hash_order']) && !empty($_POST['status'])) {
            ...
            if (!empty($order)) {
                ...
                // Allow product owner to update statuses
                if ($order->product_owner_id == $wo['user']['user_id']) {
                    ...
                    if ($order->status == 'shipped') {
                        $types = ['delivered', 'shipped'];
                    }
                }
                // Allow customer to update statuses
                elseif ($order->user_id == $wo['user']['user_id']) {
                    if ($order->status == 'placed') {
                        ...
                        $types = ['delivered', 'placed'];
                    }
                }
    
                // Check if the requested status is allowed
                if (in_array($status, $types)) {
                    $db->where('hash_id', $hash_id)->update(T_USER_ORDERS, ['status' => $status]);
    
                    // Handle specific actions for certain statuses
                    switch ($status) {
                        case 'placed': // Order reset
                            $recipient_id = $order->product_owner_id;
                            $type = 'order_reset';
                            $message = 'Order has been reset.';
                            break;
    
                        case 'accepted': // Tracking details added
                            $recipient_id = $order->user_id;
                            $type = 'added_tracking_info';
                            $message = 'Tracking information has been added.';
                            break;
    
                        case 'packed': // Delivery tracking accepted
                            $recipient_id = $order->product_owner_id;
                            $type = 'delivery_tracking_accepted';
                            $message = 'Delivery tracking accepted.';
                            break;
                        ...
                        case 'canceled': // Order canceled
                            ...
                            $db->insert(T_NOTIFICATION, $customer_notification);
                            break;
                    }
