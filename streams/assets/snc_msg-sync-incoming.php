<?php
// WoWonder → WordPress BPBM Message Inbox Bridge
...
if (!$wp_from_id || !$wp_to_id) {
    log_sync_event("INCOMING User mapping failed: WW($ww_from_id,$ww_to_id) → WP($wp_from_id,$wp_to_id)");
    http_response_code(404);
    echo json_encode(['status' => 'error', 'error' => 'User mapping failed']);
    exit;
}

// Prepare REST API call (to BPBM)
$wp_api_url = site_url('/wp-json/bp-better-messages/v1/send-message');
$body = [
...
if (is_wp_error($response)) {
    log_sync_event("INCOMING WP REST error: " . $response->get_error_message());
...
if ($code !== 200) {
    log_sync_event("INCOMING WP REST failed: code=$code; body=" . wp_remote_retrieve_body($response));
...
// Log and respond
log_sync_event("INCOMING Synced WW($ww_from_id → $ww_to_id) to WP($wp_from_id → $wp_to_id): $message");
