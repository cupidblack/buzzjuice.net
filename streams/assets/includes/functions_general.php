...
function getBrowser() {
    ...
    $known = array('Version', 'Chrome', 'Firefox', 'Safari', 'Opera', 'Edge', 'IE', 'other'); // Add known browsers
    $ub = ''; // Initialize $ub to avoid undefined variable warning

    $pattern = '#(?<browser>' . join('|', $known) . ')[/ ]+(?<version>[0-9.|a-zA-Z.]*)#';
    if (preg_match_all($pattern, $u_agent, $matches)) {
        $ub = $matches['browser'][0]; // Assign the first matched browser name to $ub
    }

    if (!empty($ub) && strripos($u_agent, "Version") < strripos($u_agent, $ub)) {
        $version = $matches['version'][0];
    } else {
        $version = $matches['version'][1] ?? '?'; // Fallback to a default value if no match is found
    }
    return array(
        'userAgent' => $u_agent,
        'name' => $bname,
...
function getBrowser($u_agent = null) {
    // Use the global $_SERVER['HTTP_USER_AGENT'] if $u_agent is not provided
    if ($u_agent === null) {
    ...
    $known = array('Version', 'Chrome', 'Firefox', 'Safari', 'Opera', 'Edge', 'IE', 'other');
    $pattern = '#(?<browser>' . join('|', $known) . ')[/ ]+(?<version>[0-9.|a-zA-Z.]*)#';

    if (preg_match_all($pattern, $u_agent, $matches)) {
        $bname = $matches['browser'][0] ?? 'Unknown';
        $version = $matches['version'][0] ?? '?';
    }
...
function watermark_image($target) {
    ...
    try {
        ...
        $image->fromFile($target)->autoOrient()->overlay("./themes/{$wo['config']['theme']}/img/icon.png", 'top left', 0.2, 10, 10)->toFile($target, 'image/jpeg');
        return true;
    }
    catch (Exception $err) {
