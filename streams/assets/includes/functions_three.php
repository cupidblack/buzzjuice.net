...
function Wo_GetGChatMembers($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_IsGChatMembers($group_id) {
    global $sqlConnect, $wo, $db;
...
function Wo_IsGChatMemberExists($group_id = false, $user_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$group_id || $group_id < 1) {
        return false;
    }
...
function Wo_AddGChatPart($group_id = false, $user_id = false) {
    ...
    if (Wo_IsGChatMemberExists($group, $user)) {
...
function Wo_GetCommonUsers($args = array()) {
    ...
    $sub_sql_string = implode(' OR ', $sub_sql);
    ...
    // Ensure sub_sql_string is not empty
    if (!empty($sub_sql_string)) {
        $sql = "
            SELECT `user_id`
            FROM $t_users
            WHERE `user_id` <> '$user'
