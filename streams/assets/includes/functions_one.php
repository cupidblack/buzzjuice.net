...
function Wo_ResetPassword($user_id, $password)
{
    ...
    // --- Use WordPress phpass for hashing password for SSO sync ---
    if (!class_exists('PasswordHash')) {
        require_once __DIR__ . '/../../../wp-includes/class-phpass.php';
    }
    $wp_hasher = new PasswordHash(8, true);
    ...
    // Safely escape for SQL
    $escaped_hash = mysqli_real_escape_string($sqlConnect, $wp_hash);

    $query = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `password` = '{$escaped_hash}' WHERE `user_id` = '{$user_id}'");
    if ($query) {
        return true;
    } else {
        return false;
...
function Wo_GetMedia($media)
    {
        ...
        // ✅ Skip prepending if already a full URL
        if (preg_match('/^(http:\/\/|https:\/\/)/i', $media)) {
            return $media;
        }
...
function Wo_PostData($post_id, $placement = '', $limited = '', $comments_limit = 0)
{
    ...
    // Modified by Blue Crown R&D for fixing the issue of postText
    $story['mentions_users'] = [];
    $mention_regex = '/@\[([0-9]+)\]/i';
    $postText = isset($story['postText']) ? $story['postText'] : '';
    if (preg_match_all($mention_regex, $postText, $matches)) { //line 6151
        foreach ($matches[1] as $match) {
            $match = Wo_Secure($match);
            $match_user = Wo_UserData($match);
            $match_search = '@[' . $match . ']';
...
function Wo_GetGroupsListAPP($fetch_array = array())
{
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['parts'] = Wo_GetGChatMembers($fetched_data['group_id']);
...
*/
function FilterStripTags($string = '') {
    return filter_var(strip_tags($string), FILTER_SANITIZE_SPECIAL_CHARS);
}
