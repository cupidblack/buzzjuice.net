...
// Add the Wo_UpdateConfig function if it doesn't already exist
if (!function_exists('Wo_UpdateConfig')) {
    function Wo_UpdateConfig($name, $value) {
        global $sqlConnect;
        ...
        $query = mysqli_query($sqlConnect, "UPDATE " . T_CONFIG . " SET `value` = '{$value}' WHERE `name` = '{$name}'");
        if ($query) {
            return true;
        } else {
            return false;
...
// Add function to update WooCommerce payment transactions
function Wo_UpdateWooPaymentTransaction($order_id, $status) {
    global $sqlConnect;
    $order_id = Wo_Secure($order_id);
    $status = Wo_Secure($status);
    
    $query = "UPDATE wo_payment_transactions SET status = '$status' WHERE order_id = '$order_id'";
    return mysqli_query($sqlConnect, $query);
}



// Add functions for creating and updating payment transactions
if (!function_exists('Wo_CreatePaymentTransaction')) {
    function Wo_CreatePaymentTransaction($user_id, $payment_type, $amount, $order_id) {
        global $sqlConnect;
        $user_id = Wo_Secure($user_id);
        $payment_type = Wo_Secure($payment_type);
        $amount = Wo_Secure($amount);
        $order_id = Wo_Secure($order_id);
        $time = time();
        
        $query = "INSERT INTO wo_payment_transactions (user_id, payment_type, amount, status, time, order_id) VALUES ('$user_id', '$payment_type', '$amount', 'pending', '$time', '$order_id')";
        return mysqli_query($sqlConnect, $query);
    }
}
...
/************ Modified by Blue Crown R&D: WoWonder WooCommerce Payment Gateway Bridge ************/
// Use existing Wo_ReplenishingUserBalance function
/*function Wo_ReplenishingUserBalance($user_id, $sum) {
    global $wo, $sqlConnect;
    ...
    $update = Wo_UpdateUserData($user_id, $update_data);
    return $update;
}
    */
...
/************ Created by Blue Crown R&D: WoWonder WooCommerce Payment Gateway Bridge ************/
// Update Wo_CreatePayment to handle WooCommerce payment transactions
function Wo_CreateWooPaymentTransaction($user_id, $amount, $order_id) {
    global $sqlConnect;
    $user_id = Wo_Secure($user_id);
    $amount = Wo_Secure($amount);
    $order_id = Wo_Secure($order_id);
    ...
    $query = "INSERT INTO wo_payment_transactions (user_id, payment_type, amount, status, time, order_id) VALUES ('$user_id', 'wow_payment', '$amount', 'pending', '$time', '$order_id')";
    return mysqli_query($sqlConnect, $query);
}
...
function Wo_DeleteProMembership() {
