...
require_once dirname(__DIR__) . '/init.php'; // Corrected path to functions_two.php
...
if (Wo_UpdateWooPaymentTransaction($order_id, 'cancelled')) {
    // Redirect to payment cancelled page
    $redirect_url = $wo['config']['site_url'] . '/payment-cancelled?order_id=' . urlencode($order_id) . '&status=cancelled';
