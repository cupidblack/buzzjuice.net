...
if (!empty($user_data['pro_type']) || !empty($user_data['pro_time']) || isset($user_data['is_pro'])) {
    // Validate pro_type
    if (!empty($user_data['pro_type']) && !is_numeric($user_data['pro_type'])) {
        $error_code = 10;
        $error_message = 'Invalid pro_type value';
    }

    // Validate pro_time
    if (!empty($user_data['pro_time']) && !is_numeric($user_data['pro_time'])) {
        $error_code = 11;
        $error_message = 'Invalid pro_time value';
    }

    // Validate is_pro
    if (isset($user_data['is_pro']) && !in_array($user_data['is_pro'], [0, 1])) {
        $error_code = 12;
        $error_message = 'Invalid is_pro value';
    }

    // If no errors, proceed with the update
    if (empty($error_code)) {
        $update_data = [];
        if (!empty($user_data['pro_type'])) {
            $update_data['pro_type'] = Wo_Secure($user_data['pro_type']);
        }
        if (!empty($user_data['pro_time'])) {
            $update_data['pro_time'] = Wo_Secure($user_data['pro_time']);
        }
        if (isset($user_data['is_pro'])) {
            $update_data['is_pro'] = Wo_Secure($user_data['is_pro']);
        }

        $update = Wo_UpdateUserData($wo['user']['user_id'], $update_data, false);
