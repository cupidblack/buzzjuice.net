...
if (!empty($_POST['type']) && in_array($_POST['type'], $required_fields)) {
    if ($_POST['type'] == 'join') {
        ...
        if (empty($error_code)) {
            if (!Wo_IsGChatMemberExists($group_id, $wo['user']['id'])) {
    ...
    if ($_POST['type'] == 'destruct_at') {
        ...
        if (empty($error_code)) {
            ...
            $group_id = Wo_Secure($_POST['id']);
            $group_tab = Wo_GroupTabData($group_id);
            if ($group_tab && is_array($group_tab) && $group_tab['type'] == 'secret' && Wo_IsGChatMemberExists($group_id, $wo['user']['id'])) {
            ...
            if (!empty($group_tab)) {
                if ($group_tab['user_id'] == $wo['user']['id']) {
                    foreach ($users as $key => $user) {
                        if (!Wo_IsGChatMemberExists($id, $user)) {
                            $active = 0;
                            if ($type == 'channel') {
                                $active = 1;
                            }
    ...
    if ($_POST['type'] == 'send') {
        ...
        if (!Wo_IsGChatMemberExists($_POST['id'], $wo['user']['id']) && $group_tab['user_id'] != $wo['user']['id']) {
            $error_code    = 13;
