...
*/
    ...
    if (!empty($session_token) || !empty($accepted_user_id) || !empty($wo['loggedin'])) {
        ...
        // REPLACEMENT: call centralized WP->Wo sync helper
        $wp_conn = function_exists('get_wp_db_conn') ? get_wp_db_conn() : null;
        $sqlConn = function_exists('get_wowonder_db') ? get_wowonder_db() : null;
        $wp_uid  = (int)($_SESSION['wp_user_id'] ?? 0);
        $wo_uid  = (int)($_SESSION['wo_user_id'] ?? 0);
        
        if ($wp_conn && $sqlConn && $wp_uid > 0 && $wo_uid > 0 && function_exists('sync_wp_user_to_wowonder_meta')) {
            try {
                sync_wp_user_to_wowonder_meta($wp_conn, $sqlConn, $wp_uid, $wo_uid);
            } catch (Throwable $e) {
                if (function_exists('bz_bridge_log')) bz_bridge_log('WP->Wo sync exception', ['ex'=>$e->getMessage(), 'wp_user_id'=>$wp_uid, 'wo_user_id'=>$wo_uid]);
            }
        } else {
            if (function_exists('bz_bridge_log')) bz_bridge_log('WP->Wo sync skipped', ['wp_conn'=> (bool)$wp_conn, 'sqlConn' => (bool)$sqlConn, 'wp_user_id'=>$wp_uid, 'wo_user_id'=>$wo_uid]);
        }
