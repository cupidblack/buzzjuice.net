...
if ($f == 'run_updater') {
    ...
    if (!empty($_GET['purchase_code'])) {
        ...
        // Bypass: Simula verificação bem-sucedida sem contatar servidor
        $check = array('status' => 'SUCCESS', 'versions' => array($version));
        if (!empty($check['status'])) {
            if ($check['status'] == 'SUCCESS') {
                if (!empty($check['versions'])) {
                    foreach ($check['versions'] as $key => $version) {
                        // Bypass: Desabilita download de atualizações do servidor oficial
                        $updater = false; // Não baixa atualizações automáticas
