<?php
if ( ! class_exists( 'myCRED_BP_Charges_License' ) ) :
	final class myCRED_BP_Charges_License {

		// Plugin Version
		public $version = BP_CHARGES_VERSION;

		// Plugin Slug
		public $slug    = BP_CHARGES_SLUG;

		public $base    = BP_CHARGES;
		...
		public function check_for_plugin_update( $checked_data ) {
			...
			$license_url = apply_filters('mycred_license_url', 'http://mycred.me/api/plugins' , $this->version, $this->slug, $this->base);


			// Start checking for an update
			$response = wp_remote_post( $license_url, $request_string );
		...
		public function plugin_api_call( $result, $action, $args ) {
			...
			$request = wp_remote_post( $license_url, $request_string );
...
function mycred_bp_charges_license() {
	return myCRED_BP_Charges_License::instance();
}
mycred_bp_charges_license();