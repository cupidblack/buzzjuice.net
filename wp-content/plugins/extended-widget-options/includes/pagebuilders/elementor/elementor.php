        ...
        if (isset($widget_options['settings']['visibility']) && isset($widget_options['settings']['visibility']['taxonomies']) && '1' == $widget_options['settings']['visibility']['taxonomies']) {
            if (!empty($widget_options['settings']['taxonomies']) && is_array($widget_options['settings']['taxonomies'])) {
                foreach ($widget_options['settings']['taxonomies'] as $tax_opt => $vallue) {
                    ...
                    $tax_label = isset($taxonomies[$tax_opt]) && isset($taxonomies[$tax_opt]->label) ? $taxonomies[$tax_opt]->label : $tax_opt;
                    $element->add_control(
                        ...
                        [
                            ...
                            'options'           => $term_array,
                            'render_type'       => 'none',
                            'description'       => __('Click on the field to search and select ' . $tax_label, 'widget-options')
                        ],
                        [
                            'overwrite'         => true
                        ]
    ...
    function widgetopts_elementor_tab_settings($element, $section_id, $args)
    {
        ...
        if ('activate' == $widget_options['logic']) {
            ...
            if (current_user_can('manage_options')) {
                $snippet_description .= '<br><a href="' . admin_url('edit.php?post_type=widgetopts_snippet') . '" target="_blank" style="display:inline-block;margin-top:5px;padding:3px 8px;background:#0073aa;color:#fff;text-decoration:none;border-radius:3px;font-size:11px;">' . __('Manage Snippets', 'widget-options') . ' &rarr;</a>';
            }
