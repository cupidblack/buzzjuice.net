...
{
    global $widget_options, $pagenow, $wp_version;
    ...
    if ('activate' == $widget_options['animation']) :
        if (!isset($widget_options['settings']['animation']) || (isset($widget_options['settings']['animation']) && !isset($widget_options['settings']['animation']['css']))) {
            wp_enqueue_style('css-animate', $css_dir . 'animate.min.css', array(), WIDGETOPTS_VERSION);
        }
    endif;
...
function widgetopts_load_admin_scripts($hook)
{
    ...
    if (($is_siteorigin) || (!$is_siteorigin && in_array($hook, apply_filters('widgetopts_load_option-tabs_scripts', array('widgets.php', 'customize.php', 'nav-menus.php'))))) {
        ...
        $btn_controls = '';
        if (isset($widget_options['clone']) && 'activate' == $widget_options['clone'] && $validLicense) {
        ...
        wp_localize_script('jquery-widgetopts-option-tabs', 'widgetopts10n', array('ajax_url' => admin_url('admin-ajax.php'), 'opts_page' => esc_url(admin_url('options-general.php?page=widgetopts_plugin_settings')), 'search_form' => $form, 'sidebaropts' => $sidebaropts, 'controls' => $btn_controls, 'translation' => array('manage_settings' => __('Enable more Widget Options superpowers', 'widget-options')), 'validate_expression_nonce' => wp_create_nonce('widgetopts-expression-nonce'), 'migration_nonce' => wp_create_nonce('widgetopts_migration_nonce'), 'migration_strings' => array('prompt_title' => __('Enter a name for the new snippet:', 'widget-options'), 'default_title' => __('Migrated Snippet', 'widget-options'), 'converting' => __('Converting...', 'widget-options'), 'success' => __('Converted successfully!', 'widget-options'), 'error' => __('Conversion failed.', 'widget-options'))));
    } else {
        wp_localize_script('widgetopts-global-script', 'widgetopts10n', array('ajax_url' => admin_url('admin-ajax.php'), 'opts_page' => esc_url(admin_url('options-general.php?page=widgetopts_plugin_settings')), 'validate_expression_nonce' => wp_create_nonce('widgetopts-expression-nonce')));
    }
    ...
    ?>
        <script>
            (function() {
                ...
                /*Classic widget option toggle*/
                jQuery(document).on('click', '.extended-widget-opts-panel', function() {
                    let expanded = jQuery(this).find('.btntoggle').attr("aria-expanded");
                    if (expanded == 'false') {
                        jQuery(this).find('.btntoggle').attr("aria-expanded", "true");
                        jQuery(this).parent().find('.extended-widget-opts-tabs').show();
                    } else {
                        jQuery(this).find('.btntoggle').attr("aria-expanded", "false");
                        jQuery(this).parent().find('.extended-widget-opts-tabs').hide();
                    }
                });

                //for accessibility mode
                jQuery(document).ready(function() {
                    if (jQuery('body.widgets_access.widgets-php .editwidget').length > 0) {

                        window.wpWidgetOpts.initAuthorDropdown(jQuery(".editwidget .extended-widget-opts-form"));
                        window.wpWidgetOpts.initRolesDropdown(jQuery(".editwidget .extended-widget-opts-form"));
                        window.wpWidgetOpts.initPageDropdown(jQuery(".editwidget .extended-widget-opts-form"));
                        window.wpWidgetOpts.initTaxonomyDropdown(jQuery(".editwidget .extended-widget-opts-form"));
                    }
                });
...
<?php }
    ...
    add_action('admin_footer-post-new.php', 'widgetopts_widgets_footer_additional_script', 999);
    add_action('admin_footer-post.php', 'widgetopts_widgets_footer_additional_script', 999);
}
