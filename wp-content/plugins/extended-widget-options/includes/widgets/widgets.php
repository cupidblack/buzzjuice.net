...
function widgetopts_in_widget_form($widget, $return, $instance)
{
    ...
    $selected = 0;
    $toggle_on = isset($_POST) && is_countable($_POST) && count($_POST) > 0 ? true : false;
    ...
    <div class="extended-widget-opts-classic extended-widget-opts-form <?php if ($width < 650 && $width > 520) {
                                                                            echo 'extended-widget-opts-form-large';
                                                                        } else if ($width <= 520) {
                                                                            echo 'extended-widget-opts-form-small';
                                                                        } ?>">
        <div class="extended-widget-opts-panel" style="padding: 7px 15px; border: 1px solid #c3c4c7; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
            <div>
                <span class="extended-widget-opts-icon" style="width: 15px; height: 15px; display: inline-block; vertical-align: middle;"></span> <strong style="vertical-align: middle; display: inline-block;">Widget Options</strong>
                <button type="button" class="btntoggle hide-if-no-js" aria-expanded="<?php echo $toggle_on === true ? 'true' : 'false'; ?>">
                    <span class="screen-reader-text">Main Sidebar</span>
                    <span class="toggle-indicator" aria-hidden="true"></span>
                </button>
            </div>
        </div>
        <div class="extended-widget-opts-tabs" style="<?php echo $toggle_on === true ? '' : 'display: none;'; ?>">
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                if ($('.so-content .extended-widget-opts-tabs').length > 0) {
                    ...
                    $('.extended-widget-opts-styling-tabs').tabs({
                        active: 0
                    });
...
<?php
    if ('activate' == $is_shortcodes && $validLicense && $widget->number != '__i__') {
    ...
    if (
        ...
        if (isset($options['extended_widget_opts'])) {
            ...
            if (isset($options['extended_widget_opts']['class']['link']) && !empty($options['extended_widget_opts']['class']['link'])) {
                if (isset($options['extended_widget_opts']['class']['http']) && !empty($options['extended_widget_opts']['class']['http'])) {
                    $options['extended_widget_opts']['class']['link'] = $options['extended_widget_opts']['class']['link'];
                } else {
...
add_action('customize_save_after', function ($object) {
