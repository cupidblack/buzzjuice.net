...
if (wp_use_widgets_block_editor()) {
	add_filter('widget_update_callback', function ($instance, $new_instance, $old_instance, $obj) {
		...
		//remove cache 
		if (isset($obj->id) && !empty($obj->id) && $obj->id != -1 && isset($widget_options) && isset($widget_options['cache']) && 'activate' == $widget_options['cache']) {
			$transient_name = 'widgetopts-cache_' . $obj->id;
...
function blockopts_filter_before_display($block_content, $parsed_block, $obj)
{
	...
	if (isset($parsed_block) && isset($parsed_block['attrs']) && (isset($parsed_block['attrs']['extended_widget_opts']) || isset($parsed_block['attrs']['extended_widget_opts_block']))) {
		...
		if (isset($widget_options['roles']) && 'activate' == $widget_options['roles'] && isset($opts['roles'])) {
			...
			$hidden = apply_filters('widget_options_visibility_roles_block', $hidden);
			if ($hidden) {
				return false;
			}
		}
		...
		if (!$isWooPage) {
			if ($is_misc && ((is_home() && is_front_page()) || is_front_page())) {
				...
				if (is_array($arrKeywords) && count($arrKeywords)) {
					...
					//do return to bypass other conditions
					$hidden = apply_filters('widget_options_visibility_slug_block', $hidden);
					if ($hidden) {
						return false;
					}
				}
		...
		if (isset($widget_options['dates']) && 'activate' == $widget_options['dates']) {
			...
			if (isset($widget_options['settings']['dates']) && isset($widget_options['settings']['dates']['days'])) {
				...
				//do return to bypass other conditions
				$hidden = apply_filters('widget_options_visibility_days_block', $hidden);
				if ($hidden) {
					return false;
				}
				//end days
			...
			if (isset($widget_options['settings']['dates']) && isset($widget_options['settings']['dates']['date_range'])) {
				...
				if (isset($dates['from']) && isset($dates['to'])) {
					if (!empty($dates['annual'])) {
						$dfrom = DateTime::createFromFormat('m/d/Y', $dates['from'], $timezone);
						$dto = DateTime::createFromFormat('m/d/Y', $dates['to'], $timezone);
					...
					//do return to bypass other conditions
					$hidden = apply_filters('widget_options_visibility_dates_block', $hidden);
					if ($hidden) {
						return false;
					}
				}
		...
		if (isset($opts['class']) && isset($opts['class']['urls']) && !empty($opts['class']['urls'])) {
			...
			if (!empty($is_url)) {
				...
				//do return to bypass other conditions
				$hidden = apply_filters('widget_options_visibility_urls_block', $hidden);
				if ($hidden) {
					return false;
				}
			}
...
function widgetopts_add_classes_post_block($block_content, $parsed_block, $obj)
{
	...
	//generate widget opts random class if widget id is missing
	$auto_generated_class = '';
	if (!isset($widget_id_set) || empty($widget_id_set)) {
		$auto_generated_class = 'widgetopts' . rand();
		$classe_to_add .= $auto_generated_class . ' ';
	}

	//add custom styling to widget
	add_action('wp_footer', function () use ($widget_id_set, $opts, $widget_options, $auto_generated_class) {
		echo widgetopts_styles_generator(empty($widget_id_set) ? $auto_generated_class : '', $opts, $widget_options, $widget_options['settings'], false, $auto_generated_class);
	});
...
function widgetopts_get_global_check_license()
{
	if (!(current_user_can('edit_pages') || current_user_can('edit_posts') || current_user_can('edit_theme_options'))) {
		die;
	}
	...
	wp_send_json_success(["validLicense" => $validLicense]);
	die;
}
add_action('wp_ajax_widgetopts_get_global_check_license', 'widgetopts_get_global_check_license');
