...
if ( is_admin() && class_exists( 'PT_Content_Views_Admin' ) ) {
	...
	class CVP_LIVE_FILTER_ADMIN {
					...
					'params'		 => array(
						array(
							...
							'params' => array(
								array(
									'label'	 => array(
										'text' => __( 'Common Options', 'content-views-pro' ),
									),
									'params' => array(
										array(
											'type'		 => 'select',
		...
		function _live_filter_settings( $prefix ) {
			...
			$heading_text						 = (CVP_LIVE_FILTER_SETTINGS::typeof_filter( $prefix ) === 'ctf-filter') ? __( 'Default label is field key.', 'content-views-pro' ) : __( 'Default label is taxonomy name.', 'content-views-pro' );
			$args[ 'live-filter-heading' ]		 = CVP_LIVE_FILTER_SETTINGS::label_text( $prefix, '', $heading_text . ' ' . __( 'Enter a space to remove label', 'content-views-pro' ) );
