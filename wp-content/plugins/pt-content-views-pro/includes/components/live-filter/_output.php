...
class CVP_LIVE_FILTER_OUTPUT {
	...
	function date_range() {
		...
		// Hidden input stores value of one/both date fields
		$field_value = $from_val;
		if ( $date_operator === 'date-fromto' ) {
			$field_value = (empty( $from_val ) && empty( $to_val )) ? "" : implode( CVP_LF_SEPARATOR, [ $from_val, $to_val ] );
		}
		$html = sprintf( '<input type="hidden" name="%s" value="%s">', esc_attr( $this->name ), esc_attr( $field_value ) );
