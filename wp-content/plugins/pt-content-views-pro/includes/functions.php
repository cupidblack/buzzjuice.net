    ...
    class PT_CV_Functions_Pro {
        ...
        static function get_google_fonts() {
            ...
            if ( file_exists( $file_path ) ) {
                ...
                add_option( PT_CV_PREFIX . 'google-fonts', $font_data, '', false );
                ...
                foreach ( $terms_info as $term_info ) {
                    if ( !empty( $term_info[ 'terms' ] ) ) {
                        ...
                        if ( $term_info[ 'operator' ] == 'NOT IN' ) {
                            ...
                            foreach ( (array) $term_info[ 'terms' ] as $term_slug ) {
        ...
        static function animate_activated_content_hover() {
            ...
            $hover_enable = apply_filters( PT_CV_PREFIX_ . 'old_overlay_enable', $hover_enable );
        ...
        static function is_ovl_layout() {
            $block_name = PT_CV_Functions::setting_value( PT_CV_PREFIX . 'blockName' );
            return method_exists( 'PT_CV_Values', 'ovl_layouts' ) && in_array( $block_name, PT_CV_Values::ovl_layouts() ) ? true : false;
        }

        static function is_pure_block() {
            return method_exists( 'ContentViews_Block', 'is_pure_block' ) && ContentViews_Block::is_pure_block();
        }
        ...
        /**
         * @since 6.2
         * @param type $has_pagination
         * @param type $is_block
         * @param type $limit
         * @return type
         */
        static function get_limit_value( $has_pagination = null, $is_block = null, $limit = null ) {
            if ( $has_pagination === null ) {
                $has_pagination = PT_CV_Functions::setting_value( PT_CV_PREFIX . 'enable-pagination' );
            }
            if ( $is_block === null ) {
                $is_block = PT_CV_Functions_Pro::is_pure_block();
            }
            if ( $limit === null ) {
                $limit = PT_CV_Functions::setting_value( PT_CV_PREFIX . 'limit' );
            }

            return (int) ($has_pagination || $is_block ? PT_CV_Functions::setting_value( PT_CV_PREFIX . 'pagination-items-per-page' ) : $limit);
        }
