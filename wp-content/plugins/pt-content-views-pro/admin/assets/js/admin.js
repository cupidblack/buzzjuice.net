...
( function ( $ ) {
	...
	$.PT_CV_Admin_Pro.prototype = {
		...
		_custom_field_filter: function () {
			...
			var toggle_placeholder = function ( $this, operator_val ) {
				...
				if ( $.inArray( operator_val, arr_comma_value_ope ) >= 0 ) {
					$value.attr( 'placeholder', 'Enter values separated by comma' );
		...
		_padding_margin: function () {
			...
			$( '.cv-padding-margin .dashicons-admin-links' ).off( 'click' ).on( 'click', function () {
				$( this ).toggleClass( 'dashicons-editor-unlink' );
			} );
			$( '.cv-padding-margin input' ).off( 'keyup change' ).on( 'keyup change', function () {
				var $wrp1 = $( this ).closest( '.cv-padding-margin' ), $icon = $wrp1.find( '.dashicons-admin-links' );
		...
		_shuffle_filter_constraint: function () {
			...
			var fn_enable_disable = function ( valid ) {
				if ( valid ) {
					var was_checked = $enable_shuffle.is( ':checked' );
					$enable_shuffle.prop( 'checked', false );
					$enable_shuffle.prop( 'disabled', true );
					if ( was_checked ) {
						// only trigger if select allowed layout & enable it, then now switch to not allowed layout
		...
		_toggle_hybrid: function () {
			...
			var $customsizeoption = $('option[value="pt-cv-custom"]');
			...
			var fn_view_type = function () {
				...
				$customsizeoption.prop('disabled',!show_animation);
