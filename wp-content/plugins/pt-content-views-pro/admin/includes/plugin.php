...
if ( !class_exists( 'PT_CV_Plugin_Pro' ) ) {
	...
	class PT_CV_Plugin_Pro {
		...
		public static function field_callback_troubleshoot_problems() {
			...
			$field_name = 'cache_ctf_list';

			PT_CV_Plugin::_field_print(
			$field_name, 'checkbox', sprintf( __( '%s Cache the custom fields list %s (to reduce loading time when editing in Admin area)', 'content-views-pro' ), '<strong>', '</strong>' ), ''
			);
