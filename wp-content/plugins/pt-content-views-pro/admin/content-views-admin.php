...
class PT_Content_Views_Pro_Admin extends PT_Content_Views_Admin {
	...
	private function __construct() {
		...
		add_filter( PT_CV_PREFIX_ . 'field_thumbnail_settings', array( $this, 'filter_field_thumbnail_settings_bottom' ), 999, 2 );
	 ...
	 * @since 7.2
	 */
	public function filter_field_thumbnail_settings_bottom( $args, $prefix ) {
		$toshow = array(
			'label'		 => array(
				'text' => '',
			),
			'params'	 => array(
				array(
					'type'		 => 'html',
					'content'	 => sprintf( '<p class="cvp-notice">%s</p>', __( "This feature is not supported when using 'Show all images in same size' or 'Soft resize'.", 'content-views-pro' ) ),
				),
			),
		);
		...
		$args[] = array(
			...
			'params' => array(
				array(
					'type'	 => 'group',
					'params' => array(
						array_merge( $toshow, array( 'dependence' => array( 'field-thumbnail-same-size', 'yes' ) ) ),
						array_merge( $toshow, array( 'dependence' => array( 'field-thumbnail-size', PT_CV_PREFIX . 'custom' ) ) ),
					),
				),
			),
			'dependence' => array( 'view-type', PT_CV_Values_Pro::hybrid_layouts( false ), '!=' ),
		);
						...
						array(
							...
							'params'		 => array(
								array(
									...
									'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Show at top left corner of image', 'content-views-pro' ) ),
									'std'		 => '',
								),
							),
	...
	public function filter_excerpt_settings( $args, $prefix ) {
		...
		$args[] = array(
			...
			'params'	 => array(
				array(
					'type'		 => 'html',
					'content'	 => sprintf( '<p class="cvp-notice" style="margin: -8px 0 25px; font-weight: 500;">%s.</p>', __( 'This option might break page layout in some themes, page builders.<br> Please select another option if problem happen', 'content-views-pro' ) ),
				),
			),
						...
						array(
							...
							'params'		 => array(
								array(
									...
									'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Show as inline link instead of button', 'content-views-pro' ) ),
									'std'		 => '',
								),
							),
						),
					...
					'params' => array(
						...
						array(
							...
							'params'	 => array(
								array(
									'type'		 => 'checkbox',
									'name'		 => $prefix . 'child-terms-ofselected',
									'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Get child terms of selected term above', 'content-views-pro' ) ),
									'std'		 => '',
									'class'		 => 'ignore',
								),
							),
			...
			$options = array(
				...
				array(
					...
					'params'		 => array(
						array(
							'type'		 => 'html',
							'content'	 => sprintf( '<p class="text-muted" style="margin-top:-12px;">%s.</p>', sprintf( __( 'Support some layouts: %s', 'content-views-pro' ), __( 'Grid (not Grid 2)', 'content-views-query-and-display-post-page' ) . ', ' . __( 'Pinterest', 'content-views-pro' ) . ', ' . __( 'Masonry', 'content-views-pro' ) . ', ' . __( 'Collapsible List', 'content-views-query-and-display-post-page' ) ) ),
					),
					),
				),
				array(
