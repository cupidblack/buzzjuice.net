...
class THWCFE_Admin_Utils extends WCFE_Checkout_Fields_Utils{
	...
	public function postmeta_form_keys($keys, $post){
		...
		if(apply_filters('thwcfe_get_postmeta_form_keys', false)){
			return $this->get_all_custom_metakeys($keys, $post);
		}
	...
	//Admin order fields adding fix not checking currect screen function and keys. WC 8.8 updates
	public function get_all_custom_metakeys($keys, $post) {
		
		if (version_compare(THWCFE_Utils::get_wc_version(), '8.8.0', '>')) {
			if (
				class_exists('\Automattic\WooCommerce\Admin\Overrides\Order') &&
				class_exists('\WC_Order') &&
				($post instanceof \WC_Order || $post instanceof \Automattic\WooCommerce\Admin\Overrides\Order)
			) {
				$checkout_type = apply_filters('thwcfe_current_checkout_type', 'classic');
				$custom_fields = $checkout_type === 'block'
					? THWCFE_Utils_Block::get_all_custom_block_checkout_fields()
					: self::get_all_custom_checkout_fields();

				$custom_field_keys = is_array($custom_fields) ? array_keys($custom_fields) : [];
				...
				if (!empty($custom_field_keys)) {
					...
					if (
						class_exists('\Automattic\WooCommerce\Internal\DataStores\Orders\OrdersTableDataStoreMeta') &&
						empty($keys)
					) {
						$limit = apply_filters('postmeta_form_limit', 30);
						$keys = wc_get_container()
							->get(\Automattic\WooCommerce\Internal\DataStores\Orders\OrdersTableDataStoreMeta::class)
							->get_meta_keys($limit);
					}

					if (!empty($keys)) {
						// Prepend custom field keys, removing any duplicates
						$keys = array_merge($custom_field_keys, array_diff((array) $keys, $custom_field_keys));
					}
				}
			}
		}
