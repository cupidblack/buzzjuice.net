...
abstract class THWCFE_Admin_Form {
	...
	public function render_form_fragment_rules_ajax($type="field", $checkout_type = 'classic'){
		?>
                                                        	...
                                                        	<td class="thpladmin_rule_operand operand">
                                                            	...
                                                            	<?php 
																	if($checkout_type === 'block'){
																		$this->render_form_fragment_fields_select_block();
																	}else{
																		$this->render_form_fragment_fields_select(); 
																	}
	...
	private function render_form_fragment_fields_select_block(){

		$sections = THWCFE_Utils_Block::get_block_checkout_sections();
		$show_name = apply_filters('thwcfe_show_filed_name_for_field_list_in_conditions_tab', true);

		$other_fields = array('use-same-address-for-billing-checkbox' => 'Use same address for billing');
		if(THWCFE_Utils::get_settings('enable_conditions_payment_shipping')){
			$other_fields['shipping_method[0]'] = 'Shipping Method';
			$other_fields['payment_method'] = 'Payment Method';
		}
				...
				foreach($sections as $sname => $section){
					...
					if($section && THWCFE_Utils_Section::is_valid_section($section)){
						...
						if($sname === 'address') {
							...
							// Duplicate as Shipping
							echo '<optgroup label="Shipping">';
							foreach ($fields as $name => $field) {
								if ($field && THWCFE_Utils_Field::is_valid_field($field) && THWCFE_Utils_Field::is_enabled($field)) {
									$label = $field->get_property('title');
									$label = empty($label) ? $name : $label;
							...
							echo '</optgroup>';
							// Duplicate as Billing
							if ($fields && is_array($fields)) {
								echo '<optgroup label="Billing">';
								foreach ($fields as $name => $field) {
									if ($field && THWCFE_Utils_Field::is_valid_field($field) && THWCFE_Utils_Field::is_enabled($field)) {
										$label = $field->get_property('title');
										$label = empty($label) ? $name : $label;
							...
							if($fields && is_array($fields)){
								echo '<optgroup label="' . esc_html($title) . '">';
								foreach($fields as $name => $field){
									if($field && THWCFE_Utils_Field::is_valid_field($field) && THWCFE_Utils_Field::is_enabled($field)){
										$label = $field->get_property('title');
										$label = empty($label) ? $name : $label;
	...
	public function render_field_form_fragment_fields_wrapper( $checkout_type = 'classic' ){
		?>
