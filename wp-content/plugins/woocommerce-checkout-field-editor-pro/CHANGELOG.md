...
IMPROVED: Added price option support for section fields in the block checkout.
IMPROVED: Added required validation for the checkbox field in the block checkout (WC 9.8+).
IMPROVED: Added a new filter to display all custom fields when an admin creates an order.
IMPROVED: Added compatibility with WooCommerce 9.9.
IMPROVED: Added compatibility with WordPress 6.8.
FIX: Fixed shipping calculation issue when "Hide shipping costs until an address is entered" is enabled.

#### 3.7.3.0
IMPROVED: Added Date Picker (React) field to the block checkout.
IMPROVED: Added support for JS dynamic string translation in block checkout using WPML.
