...
class THWCFE_Public_Checkout extends THWCFE_Public {
	...
	private function enqueue_scripts($suffix, $jquery_version, $in_footer) {
		...
		$shipping_visible_after_adrs = (
			$address_required && 
			version_compare(THWCFE_Utils::get_wc_version(), '9.8.0', ">=")
		);
		$wcfe_var = array(
			'lang' => array(
	...
	public function define_public_hooks(){
		...
		if(version_compare(THWCFE_Utils::get_wc_version(), '9.8.0', ">=")){
			/** Both shipping and billing fields should have the same required functionality **/
			add_filter('woocommerce_get_country_locale', array($this, 'modify_address_fields'),$hp_cf);
			add_filter('woocommerce_get_country_locale_default', array($this, 'make_address_fields_default'),$hp_cf);
		}
	}
	...
	*/
	public function modify_address_fields($locales) {
		...
		$section_name = $use_shipping_address ? 'shipping' : 'billing';
		$section = $this->get_checkout_section($section_name);
		$field_set = THWCFE_Utils_Section::get_fieldset($section);
