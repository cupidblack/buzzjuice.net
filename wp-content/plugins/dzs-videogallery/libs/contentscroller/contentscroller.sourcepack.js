...
class DzsContentScroller {
  ...
  initClass() {
    ...
    selfClass.cthis = $(selfClass.argThis);
    ...
    function setup_structure() {
      ...
      if (selfClass.cthis.find('.thumbsClip').length === 0) {
    ...
    function reinitParseItems() {
      ...
      _items.children('.csc-item').each(function () {
        var $item = $(this);
        var ind2 = $item.parent().children().index($item);


        // -- each csc item
        var _c2 = $item.find('.divimage,img');
        if (_c2.length && _c2.eq(0).attr('data-src')) {
          $item.addClass('needs-loading');
        ...
        $item.wrapInner('<div class="csc-item--inner"></div>');
        ...
        if (o.settings_autoHeight_proportional_max_height) {

          $item.css({
            'maxHeight': o.settings_autoHeight_proportional_max_height + 'px'
          });
        }

        selfClass.$thumbsCon.append($item);

        if (o.settings_lazyLoading === 'on') {
          if ($item.find('.imagediv').length === 0 && $item.find('img.imageimg').length === 0) {
    ...
    function loadedImage(pargs) {
      ...
      if (pargs) {
        loadedImageArgs = $.extend(loadedImageArgs, pargs);
        if (pargs.currentTarget) {
          loadedImageArgs.target = pargs.currentTarget;
          if (loadedImageArgs.target && loadedImageArgs.target.dzscsc_index) {
            loadedImageArgs.dzscsc_index = loadedImageArgs.target.dzscsc_index;
          }
        }
      }


      if (loadedImageArgs.dzscsc_index) {
        ind = loadedImageArgs.dzscsc_index;
      }
      if (loadedImageArgs.target) {
        _t = $(loadedImageArgs.target);
      }
      if (loadedImageArgs._cscItem) {
        _cscItem = $(loadedImageArgs._cscItem);
      }
    ...
    function view_removePreInitDimensions() {
    ...
    function dimensionsCalculateTotalSize() {
    ...
    function init_allLoaded() {
      ...
      selfClass.cthis.children('.preloader, .preloader-semicircles').addClass('preloader-hidden');
    ...
    function handle_mouse(e) {
      ...
      if (e.type === 'mouseover') {
        if (_t.hasClass('bullet')) {
          ...
          let ind = _t.parent().children().index(_t);
          if (selfClass.cthis.find(_t).length < 1) {
            return;
          }
    ...
    function gotoPage(targetIndex, pargs) {
      ...
      if (margs.called_from_resize === false && action_call_on_gotoItem) {
        selfClass.cthis.get(0).api_do_transition = view_doTransition;
    ...
    function view_doTransition(pargs) {
  ...
  $.fn.contentscroller = function (argOptions) {
    ...
    if (typeof argOptions === 'undefined') {
      if (typeof $(this).attr('data-options') !== 'undefined') {
        let dataOptionsString = $(this).attr('data-options');
        try {
          var dataOptionsString_json = JSON.parse(dataOptionsString);
          argOptions = $.extend({}, dataOptionsString_json);

        } catch (err) {
          console.error(err);

          dataOptionsString = $(this).attr('data-options');
          dataOptionsString = 'window.dzscsc_self_options  = ' + dataOptionsString;
          try {
            eval(dataOptionsString);
          } catch (err) {
