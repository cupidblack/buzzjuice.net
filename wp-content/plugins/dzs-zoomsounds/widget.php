    ...
    if (is_array($term_list) && count($term_list) > 0) {
      ...
      if (isset($_GET[DZSAP_VIEW_QUERY_PARAMS_TAGS_FILTER]) && $_GET[DZSAP_VIEW_QUERY_PARAMS_TAGS_FILTER]) {
        ...
        $fout .= add_query_arg(array(
          DZSAP_VIEW_QUERY_PARAMS_TAGS_FILTER => ''
        ), dzs_curr_url());
        $fout .= '">';
        $fout .= '<small>';
        $fout .= '&#8592; ' . esc_html__("All tags", 'dzsap');
        $fout .= '</small>';
        $fout .= '</a>';
        $fout .= '</li>';
      }
      ...
      foreach ($term_list as $lab => $term) {
        ...
        $fout .= add_query_arg(array(
          DZSAP_VIEW_QUERY_PARAMS_TAGS_FILTER => $cach_tag->slug
        ), dzs_curr_url());
  ...
  public function form($instance) {
    ...
    <div>
      ...
      <input type="text" name="<?php echo $this->get_field_name('title') ?>"
             id="<?php echo $this->get_field_id('title') ?> " value="<?php echo $margs['title'] ?>" size="20"/>
    </div>
