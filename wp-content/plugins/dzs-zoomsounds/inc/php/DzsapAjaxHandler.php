...
include_once(DZSAP_BASE_PATH . 'inc/php/db/pcm-functions.php');

class DzsapAjaxHandler {

  public $dzsap = null;
  /**
   * DzsapAjaxHandler constructor.
  ...
  function register_actions() {
    ...
    add_action('wp_ajax_dzsap_' . DZSAP_ANALYTICS_SUBMIT_VIEW_NAME, array($this, 'submit_views'));
    add_action('wp_ajax_nopriv_dzsap_' . DZSAP_ANALYTICS_SUBMIT_VIEW_NAME, array($this, 'submit_views'));
  ...
  function admin_nag_disable_all() {
    ...
    update_option(DZSAP_DBNAME_MAINOPTIONS, $this->dzsap->mainoptions, false);
    die();
  }
  ...
  function check_init_args() {
    ...
    if (isset($_GET['dzsap_action']) && $_GET['dzsap_action'] == DZSAP_AJAX_LOAD_CHARTS_HTML) {
      ...
      if ($_POST['action'] == 'dzsap_duplicate_dzsap_configs') {
        ...
        if (isset($_POST['slidernr'])) {
          if (isset($_GET['page']) && $_GET['page'] == DZSAP_ADMIN_PAGENAME_LEGACY_SLIDERS_ADMIN_VPCONFIGS) {
            ...
            update_option(DZSAP_DBNAME_AUDIO_PLAYERS_CONFIGS, $dzsap->mainitems_configs, false);

            wp_redirect(admin_url('admin.php?page=' . DZSAP_ADMIN_PAGENAME_LEGACY_SLIDERS_ADMIN_VPCONFIGS . '&currslider=' . $dzsap->currSlider));
            exit;
          }
        }
      }
...
// Update the post into the database
  ...
  function submit_download() {
    ...
    dzsap_mysql_insert_activity(array(
      'id_video' => $playerid,
      'playerSource' => $_POST['playerSource'],
      'type' => 'download',
    ));
  ...
  function submit_views() {
    ...
    if (isset($_COOKIE[DZSAP_COOKIENAME_ANALYTICS_VIEW_SUBMITTED . '-' . $playerid])) {
    ...
    setcookie(DZSAP_COOKIENAME_ANALYTICS_VIEW_SUBMITTED . "-" . $playerid, '1', time() + (intval($dzsap->mainoptions['play_remember_time']) * 60), COOKIEPATH, $_SERVER['HTTP_HOST'], DZSAP_COOKIES_SECURE, true);
  ...
  function submit_rate() {
    ...
    if (get_post_meta($playerid, DZSAP_VIEW_PLAYER_FEATURE_RATINGS_METANAME_RATEINDEX, true) != '') {
      $rate_index = intval(get_post_meta($playerid, DZSAP_VIEW_PLAYER_FEATURE_RATINGS_METANAME_RATEINDEX, true));
    }
    ...
    update_post_meta($playerid, DZSAP_VIEW_PLAYER_FEATURE_RATINGS_METANAME_RATEINDEX, $rate_index);
  ...
  function submit_pcm() {
    ...
    if ($_POST['playerid']) {
      $playerId = (DZSZoomSoundsHelper::sanitize_toKey($_POST['playerid']));
    }

    if ($_POST['source']) {
      $playerSource = (DZSZoomSoundsHelper::sanitize_toKey($_POST['source']));
    }
    ...
    if ((isset($_POST['call_from']) && $_POST['call_from'] = 'manual_wave_overwrite') || strpos($pcmData, ',') !== false) {
      ...
      dzsap_db_savePcmData($pcmData, $playerId, $forceUrlTrackId, $playerSource);
...
//      echo json_encode($responseArray);
  ...
  function remove_sample_tracks() {
    ...
    update_option(DZSAP_DBNAME_SAMPLEDATA, $dzsap->sample_data, false);
      ...
      foreach ($resp['items'] as $lab => $it) {
        ...
        if ($it['type'] == 'slider_import') {
          ...
          if ($sw_import) {
            ...
            update_option(DZSAP_DBNAME_MAINITEMS, $dzsap->mainitems, false);
          }
        }
  ...
  function delete_notice() {
    ...
    update_option($_POST['postdata'], 'seen', false);
    die();
  }
  ...
  function deactivate_license() {
    ...
    update_option(DZSAP_DBNAME_OPTIONS, $dzsap->mainoptions, false);
  ...
  function delete_pcm() {
    ...
    $dbPcmKey = DZSAP_DB_PCM_DATA . '_' . (DZSZoomSoundsHelper::sanitize_toKey($playerId));
    ...
    if ($trackSource) {
      ...
      $dbPcmKey = DZSAP_DB_PCM_DATA . '_' . (DZSZoomSoundsHelper::sanitize_toKey($trackSource));
  ...
  function saveLegacyitems() {
    ...
    if (isset($_POST['sliderid'])) {
      ...
      $legacyMainArray = get_option($dbname_mainitems);
      foreach ($auxarray as $label => $value) {
        $aux = explode('-', $label);
        $tempmainarray[$aux[1]][$aux[2]] = $auxarray[$label];
      }
      $legacyMainArray[$_POST['sliderid']] = $tempmainarray;
    } else {
      foreach ($auxarray as $label => $value) {

        $aux = explode('-', $label);
        $legacyMainArray[$aux[0]][$aux[1]][$aux[2]] = $auxarray[$label];
      }
    }
    echo $dbname_mainitems;
    echo isset($_POST['currdb']);
    update_option($dbname_mainitems, $legacyMainArray, false);
    echo 'success';
    die();
  }
  ...
  function save_audioplayer_configs() {
    ...
    if (isset($_POST['sliderid'])) {
      ...
      foreach ($dataArrayFromPost as $label => $value) {
        $aux = explode('-', $label);
        $tempMainArrayVpConfigs[$aux[1]][$aux[2]] = $dataArrayFromPost[$label];
      }

      $mainarray[$_POST['sliderid']] = $tempMainArrayVpConfigs;
    } else {
      ...
      if (isset($_POST['slider_name'])) {
        ...
        if ($_POST['slider_name'] == DZSAP_VPCONFIGS_PREVIEW_IFRAME_VPCONFIG_NAME) {
          update_option(DZSAP_VPCONFIGS_PREVIEW_IFRAME_VPCONFIG_NAME_IN_DB, $tempMainArrayVpConfigs);
        } else {

          $mainarray[$vpconfig_k] = $tempMainArrayVpConfigs;
        }
    ...
    update_option(DZSAP_DBNAME_AUDIO_PLAYERS_CONFIGS, $mainarray, false);
  ...
  function save_mainOptions() {
    ...
    if (isset($mainOptionsFromPost[DZSAP_PERMISSION['PORTAL_ENABLE_UNREGISTERED_SUBMIT']]) && $mainOptionsFromPost[DZSAP_PERMISSION['PORTAL_ENABLE_UNREGISTERED_SUBMIT']] == 'on') {
    ...
    if (isset($mainOptionsFromPost[DZSAP_PERMISSION['PORTAL_EDIT_OWN_TRACKS']]) && $mainOptionsFromPost[DZSAP_PERMISSION['PORTAL_EDIT_OWN_TRACKS']] == 'on') {

      $role = get_role('subscriber');

      $role->remove_cap('edit_posts');
      $role->remove_cap(DZSAP_REGISTER_POST_TYPE_NAME);
  ...
  function create_activity_table() {
    ...
    update_option('dzsap_table_activity_created', 'on', false);
  }
