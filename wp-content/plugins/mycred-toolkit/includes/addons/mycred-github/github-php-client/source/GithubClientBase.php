...
abstract class GitHubClientBase {
	...
	public function getLastPage() {
		if (!isset($this->pageData['last'])) {
			throw new GitHubClientException('Last page not defined', esc_attr( GitHubClientException::PAGE_INVALID ) );
		}
	...
	public function getNextPage() {
		...
		if (is_null($this->page)) {
			throw new GitHubClientException('Page not defined', esc_attr( GitHubClientException::PAGE_INVALID ) );
		}
	...
	protected function doRequest( $url, $method, $data, $contentType = null, $filePath = null ) {
		...
		if ($this->debug) {
			...
			echo 'Data: ' . esc_html(print_r($data, true)) . "\n";
		}
	...
	public function request( $url, $method, $data, $expectedHttpCode, $returnType, $isArray = false ) {
		...
		if (is_array($data) && !is_null($this->page)) {
			if (!is_numeric($this->page) || $this->page <= 0) {
				$this->resetPage();
				throw new GitHubClientException('Page must be positive value', esc_attr( GitHubClientException::PAGE_INVALID) );
			}

			if (!is_numeric($this->pageSize) || $this->pageSize <= 0 || $this->pageSize > 100) {
				$this->resetPage();
				throw new GitHubClientException('Page size must be positive value, maximum value is 100', esc_attr(GitHubClientException::PAGE_SIZE_INVALID) );
			}
	...
	public function parseResponse( $url, $response, $returnType, $expectedHttpCode, $isArray = false ) {
		...
		if ($status !== $expectedHttpCode) {
			throw new GitHubClientException(
				sprintf(
					'Expected status [%s], actual status [%s], URL [%s]',
					esc_html($expectedHttpCode),
					esc_html($status),
					esc_url_raw($url)
				),
				esc_attr( GitHubClientException::INVALID_HTTP_CODE
			) );
		}
