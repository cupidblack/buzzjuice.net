...
function Learndash_Join_Group_myCRED_Hook( $installed ) {
	...
	$installed['hook_join_group_learndash'] = array(
		'title' => __('Join Group (LearnDash)', 'mycred-toolkit'),
		...
		class myCRED_Hook_Learndash_Join_Group extends myCRED_Hook {
			...
			function __construct( $hook_prefs, $type = 'mycred_default' ) {
				parent::__construct(array(
					...
					'defaults' => array(
						'creds' => 0,
						'log' => __('%plural% for Joining a Group', 'mycred-toolkit'), 
						'limit' => '0/x',
				...
				if ( $prefs['check_specific_hook'] == '1' && !empty( $prefs['join_group']['join_group_select_group'] ) && in_array( $group_id, $prefs['join_group']['join_group_select_group'] ) ) {
				   $hook_index = array_search( $group_id, $prefs['join_group']['join_group_select_group'] );
					...
					!empty( $prefs['join_group']['join_group_select_group'] ) && isset( $prefs['join_group']['join_group_select_group'][ $hook_index ] )
						...
						if ( $this->core->has_entry( 'learndash_join_group', $group_id, $user_id) ) {
return;
						}
			...
			public function preferences() {
				...
				<?php 
				  ...
				  $join_group_data = array(
					  array(
						  'creds' => 0,
						  'log' => __('%plural% for Joining Group', 'mycred-toolkit'),
						...
						<div class="group_custom_hook_class">
							<div class="row">
								...
								<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
									<div class="form-group">
										<label><?php esc_html_e( 'Select Specific Group', 'mycred-toolkit' ); ?></label>
										...
										$group_id = $group_name->ID;
											...
											echo '<option class="select-value" value="' . esc_attr( $group_name->ID ) . '" ' . ( $group_name->ID == $label['join_group_select_group'] ? ' selected' : '' ) . ' >' . esc_attr( $group_name->post_title ) . '</option>';
										}
								...
								<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<div class="form-group">
										<label for="<?php echo $this->field_id(array( 'join_group' => 'log' )); ?>"><?php esc_html_e('Log Template', 'mycred-toolkit'); ?></label>
