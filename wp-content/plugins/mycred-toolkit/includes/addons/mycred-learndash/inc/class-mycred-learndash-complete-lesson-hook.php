...
function Learndash_Completing_Lesson_myCRED_Hook( $installed ) {
	...
	$installed['hook_completing_lesson_learndash'] = array(
		'title' => __('Completing a Lesson (Learndash) ', 'mycred-toolkit'),
		...
		class myCRED_Hook_Learndash_Completing_Lesson extends myCRED_Hook {
			...
			function __construct( $hook_prefs, $type = 'mycred_default' ) {
				parent::__construct(array(
					...
					'defaults' => array(
						'creds' => 0,
						'log' => __('%plural% for Completing a General Lesson', 'mycred-toolkit'), 
						'limit' => '0/x',
			  ...
			  $terms =  get_the_terms( $args['lesson']->ID, 'ld_lesson_tag');
				...
				if (  isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_lesson_completed']['select_lesson'] ) && in_array( $args['lesson']->ID, $prefs['specific_lesson_completed']['select_lesson'] )  ) {
					...
					if (  
						...
						if ( $this->core->has_entry( 'learndash_lesson_complete', $args['lesson']->ID, $args['user']->ID) ) {
return;
						}
					...
					if ($this->over_hook_limit('specific_lesson_completed', 'learndash_lesson_complete', $args['user']->ID)) {
					 ...
					 echo '<option class="select-value" value="' . esc_attr( $tag->term_id ) . '" ' . ( $tag->term_id == $prefs['specific_lesson_completed']['select_lesson'] ? ' selected' : '' ) . ' >' . esc_attr( $tag->name ) . '</option>';
			...
			public function preferences() {
				...
				<?php 
					...
					$lesson_complete_data = array(
						array(
							'creds' => 0,
							'log' => __('%plural% for Completing a Specific Lesson', 'mycred-toolkit'),
							'limit' => '0/x',
							'select_option' => 0,
							'select_lesson' => 0,
							'select_tag' => 0,     
					   ...
					   <?php 
						...
						<div class="lesson_custom_hook_class">
							<div class="row">
								...
								<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
									<div class="form-group">
										<label><?php esc_html_e( 'User Selected Option', 'mycred-toolkit' ); ?></label>
										...
										if ($label['select_option'] == 'lesson') {
											...
											$lesson_id = $lesson_name->ID;
												...
												echo '<option class="select-value" value="' . esc_attr( $lesson_name->ID ) . '" ' . ( $lesson_name->ID == $label['select_lesson'] ? ' selected' : '' ) . ' >' . esc_attr( $lesson_name->post_title ) . '</option>';
											}
											...
											foreach ($tags as $tag) {
												...
												$lesson_field = $this->lesson_name();
												 ...
												 echo '<option class="select-value" value="' . esc_attr( $tag->term_id ) . '" ' . ( $tag->term_id == $label['select_lesson'] ? ' selected' : '' ) . ' >' . esc_attr( $tag->name ) . '</option>';
								...
								<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
									<div class="form-group">
										<label for="<?php echo $this->field_id(array( 'specific_lesson_completed' => 'log' )); ?>"><?php esc_html_e('Log Template', 'mycred-toolkit'); ?></label>
