...
function Learndash_Completing_Topic_myCRED_Hook( $installed ) {
	...
	$installed['hook_completing_topic_learndash'] = array(
		'title' => __('Completing a Topic (Learndash) ', 'mycred-toolkit'),
		...
		class myCRED_Hook_Learndash_Completing_Topic extends myCRED_Hook {
			...
			function __construct( $hook_prefs, $type = 'mycred_default' ) {
				parent::__construct(array(
					...
					'defaults' => array(
						'creds' => 0,
						'log' => __('%plural% for Completing a General Topic', 'mycred-toolkit'), 
						'limit' => '0/x',
				...
				if (  isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_topic_completed']['select_topic'] ) && in_array( $args['topic']->ID, $prefs['specific_topic_completed']['select_topic'] )  ) {
				  ...
				  $hook_index = array_search( $args['topic']->ID, $prefs['specific_topic_completed']['select_topic'] );
					...
					if (  
						...
						if ( $this->core->has_entry( 'learndash_topic_complete', $args['topic']->ID, $args['user']->ID) ) {
return;
						}
...
return;
			...
			public function mycred_learndash_arrange_data( $specific_hook_data ) {
                ...
                $select_topic = bzj_safe_array($specific_hook_data['select_topic']);
                ...
                foreach ( $creds as $key => $value ) {
                    ...
                    $hook_data[$key]['select_topic']  = $select_topic[$key] ?? '';
			...
			public function preferences() {
				...
				<?php 
					 ...
					 $topic_complete_data = array(
						 array(
							 'creds' => 0,
							 'log' => __('%plural% for Completing a Specific Topic', 'mycred-toolkit'),
							 'limit' => '0/x',
							 'select_option' => 0,
							 'select_topic' => 0,
							 'select_tag' => 0,
						...
						<div class="topic_custom_hook_class">
							<div class="row">
									...
									<div class="form-group">
										...
										if ($label['select_option'] == 'topic') {
											...
											$topic_id = $topic_name->ID;
												...
												echo '<option class="select-value" value="' . esc_attr( $topic_name->ID ) . '" ' . ( $topic_name->ID == $label['select_topic'] ? ' selected' : '' ) . ' >' . esc_attr( $topic_name->post_title ) . '</option>';
											}
											...
											foreach ($tags as $tag) {
												...
												$topic_field = $this->topic_name();
												 ...
												 echo '<option class="select-value" value="' . esc_attr( $tag->term_id ) . '" ' . ( $tag->term_id == $label['select_topic'] ? ' selected' : '' ) . ' >' . esc_attr( $tag->name ) . '</option>';
								...
								<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
									<div class="form-group">
										<label for="<?php echo $this->field_id(array( 'specific_topic_completed' => 'log' )); ?>"><?php esc_html_e('Log Template', 'mycred-toolkit'); ?></label>
