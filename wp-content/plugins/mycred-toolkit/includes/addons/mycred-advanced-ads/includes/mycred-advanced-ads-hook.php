...
/**
 * Hook for Advanced Ads Publication
 * @since 1.0
 * @version 1.0
 */
if (!class_exists('myCRED_Advanced_Ads_Hook')):
    class myCRED_Advanced_Ads_Hook extends myCRED_Hook
    {
        ...
        function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY)
        {
            ...
            parent::__construct(array(
                'id' => 'advanced_ads_publication',
                'defaults' => array(
                    'creds' => 1,
                    'log' => __('%plural% for publishing an ad', 'mycred-toolkit'),
                    'limit' => '0/x'
                )
            ), $hook_prefs, $type);
        }
        ...
        public function run()
        {
            add_action( 'advanced-ads-ad-status-published', array( $this, 'mycred_advanced_ads_ad_published'), 10 );
        }

        /**
         * Award points for Advanced Ads Publication
         */
        public function mycred_advanced_ads_ad_published( $ad ) {

            // Extract ad ID
            // Assuming $ad is an object as per user's snippet
            $ad_id = $ad->get_id(); // The user snippet had $ad->get_id( $ad ) which looks slightly off if it's a method on $ad, but I'll follow common patterns if not sure. Actually user said $ad_id = $ad->get_id( $ad );

            if ( method_exists( $ad, 'get_id' ) ) {
                $ad_id = $ad->get_id();
            } else {
                // Fallback attempt if it's not a standard object as expected
                return;
            }
            ...
            $reference = 'advanced_ads_publication';
        ...
        /**
         * Preference for Advanced Ads Hook
         * @since 1.0
         * @version 1.0
         */
        public function preferences()
        {
            ...
            <div class="hook-instance">
                <h3><?php esc_html_e('Publishing an Ad', 'mycred-toolkit'); ?></h3>
                <div class="row">
