 ...
 */
if ( ! class_exists( 'myCRED_FRM_Form' ) ) :
	class myCRED_FRM_Form extends myCRED_Hook {
		...
		public function run() {
			...
			if ( isset($this->prefs['mycred_frm_action']) && !empty($this->prefs['mycred_frm_action']) && is_array($this->prefs['mycred_frm_action']) ) {
				add_action( 'forminator_custom_form_submit_before_set_fields', array( $this, 'on_frm_form_submission' ), 100, 3 );
				add_action( 'forminator_cform_user_registered', array( $this, 'mycred_forminator_form_submission' ), 200, 4 );
			}
			
		}

		public function mycred_forminator_form_submission($user_id, $custom_form, $entry,$new_user_data) {

			if ( $this->core->exclude_user( $user_id ) ) return;

			$prefs = $this->prefs;
			$form_id = $custom_form->id;
			$field_data = $custom_form->fields;
			
			if ( !empty($prefs) && is_array($prefs) ) {
				$refrence = 'frm_form_submit';

				for( $i =0; $i<count($prefs['mycred_frm_action']); $i++) {

					if ( 'frm_submit_frm' == $prefs['mycred_frm_action'][$i] && !empty($prefs['creds'][$i]) && !$this->frm_over_hook_limit( $i, 'frm_submit_frm', $user_id )) {
					...
					} else if( 'frm_submit_spec_frm' == $prefs['mycred_frm_action'][$i] && !$this->frm_over_hook_limit( $i, 'frm_submit_spec_frm', $user_id )) {
						
						if (isset($prefs['mycred_frm_form'][$i]) && !empty($prefs['mycred_frm_form'][$i]) && $prefs['mycred_frm_form'][$i] == $custom_form->id) {
							$data = array (
								'mycred_frm_action' => $prefs['mycred_frm_action'][$i],
								'form_id' => $custom_form->id,
							);
					...
					} else if( 'frm_submit_spec_field_frm' == $prefs['mycred_frm_action'][$i] && !$this->frm_over_hook_limit( $i, 'frm_submit_spec_field_frm', $user_id )) {
						...
						isset($prefs['mycred_frm_field_val'][$i]) && !empty($prefs['mycred_frm_field_val'][$i])) {
							...
							foreach ($custom_form->id as $field) {
					...
					} else if( 'frm_submit_spec_field_spec_frm' == $prefs['mycred_frm_action'][$i] && !$this->frm_over_hook_limit( $i, 'frm_submit_spec_field_spec_frm', $user_id )) {


						if (isset($prefs['mycred_frm_form'][$i]) && $prefs['mycred_frm_form'][$i] == $form_id &&
						...
						isset($prefs['mycred_frm_field_val'][$i]) && !empty($prefs['mycred_frm_field_val'][$i])) {
							...
							foreach ($custom_form->fields as $field) {
								if ($field['name'] == $prefs['mycred_frm_field_name'][$i] && $field['value'] == $prefs['mycred_frm_field_val'][$i]) {
									...
									$this->core->add_creds(
										'frm_submit_spec_field_spec_frm',
										$user_id,
										$prefs['creds'][$i],
										$prefs['log'][$i],
										$form_id,
		...
		/**
		 * Runs when a forminator form is submitted
		 */
		public function on_frm_form_submission( $entry, $form_id, $field_data ) {
			...
			if ( !empty($prefs) && is_array($prefs) ) {
				...
				for( $i =0; $i<count($prefs['mycred_frm_action']); $i++) {
					...
					if ( 'frm_submit_frm' == $prefs['mycred_frm_action'][$i] && !empty($prefs['creds'][$i]) && !$this->frm_over_hook_limit( $i, 'frm_submit_frm', $user_id )) {
						...
						if (isset($prefs['mycred_frm_form'][$i]) && !empty($prefs['mycred_frm_form'][$i]) && $prefs['mycred_frm_form'][$i] == $form_id) {
							$data = array (
								'mycred_frm_action' => $prefs['mycred_frm_action'][$i],
								'form_id' => $form_id,
							);
		...
		*/
			...
			if ( isset($prefs['creds']) && count( $prefs['creds'] ) > 0 ) {
				...
				$default_data = array(
					array(
						'creds'   => 1,
						'limit'   => 'x',
						'log'   => '%plural% for Forminator form submit',
						'mycred_frm_form'   => '',
		...
		public function mycred_frm_arrange_hook_data( $data ){
			...
			foreach ( $data['mycred_frm_action'] as $key => $value ) {
				...
				$hook_data[$key]['mycred_frm_form'] = $data['mycred_frm_form'][$key];
		...
		public function mycred_frm_hook_settings($data, $obj) {
			$actions = array(
				'frm_submit_frm' => 'Submit a form',
				'frm_submit_spec_frm' => 'Submit a specific form',
				'frm_submit_spec_field_frm' => 'Submit a specific field value on any form',
				'frm_submit_spec_field_spec_frm' => 'Submit a specific field value on a specific form'
			);
			...
			<div class="hook-instance">
				...
				<div class="row mycred-frm-form-fields">
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<div class="form-group frm-actions">
							<label ><?php esc_html_e( 'Actions', 'mycred' ); ?></label>
							<select class="form-control mycred-frm-form-actions" name="<?php echo $this->mycred_frm_field_name( $obj->mycred_type, 'mycred_frm_action' ); ?>">
								<?php
					...
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<div class="form-group frm-forms">
							<label ><?php esc_html_e( 'Forms', 'mycred' ); ?></label>
							<select class="form-control mycred-frm-forms" name="<?php echo $this->mycred_frm_field_name( $obj->mycred_type, 'mycred_frm_form' ); ?>">
								<?php
								foreach ($forms as $form ) {
									$selected = '';
									if ( $form->id == $prefs['mycred_frm_form']) {
										$selected = 'Selected';
									}
									?>
				...
				<div class="row">
					<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
						<div class="form-group frm-forms-fields">
							<label ><?php esc_html_e( 'Field Name', 'mycred' ); ?></label>
							<input type="text" value="<?php echo esc_attr( $prefs['mycred_frm_field_name'] ); ?>" name="<?php echo $this->mycred_frm_field_name( $obj->mycred_type, 'mycred_frm_field_name' ); ?>" class="form-control frm-forms-fields-name">
						</div>
					</div> 
					<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
						<div class="form-group frm-forms-fields">
							<label ><?php esc_html_e( 'Field Value', 'mycred' ); ?></label>
							<input type="text" value="<?php echo esc_attr( $prefs['mycred_frm_field_val'] ); ?>" name="<?php echo $this->mycred_frm_field_name( $obj->mycred_type, 'mycred_frm_field_val' ); ?>" class="form-control frm-forms-fields-val">
						</div>
					</div>
				</div>
				</div>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group mycred-frm-specific-hook-actions textright">
								...
								if ($prefs === end($data)) {
									?>
									<button class="button button-small mycred-frm-add-hook" type="button">Add More</button>
									<?php
								} 
								?>
								<?php 
								//if ( $count >= 1) {
									?>
									<button class="button button-small mycred-frm-remove-hook" type="button">Remove</button>
									<?php
								//} 
								?>
							</div>
		...
		public function mycred_frm_field_name( $type, $attr ){
			...
			return "{$hook_prefs_key}[hook_prefs][frm_form_submit][{$attr}][]";
		}
		...
		public function sanitise_preferences( $data ) {
			...
			foreach ( $data as $data_key => $data_value ) {
				foreach ( $data_value as $key => $value) {
					...
					else if ( $data_key == 'mycred_frm_form' ) {
						$new_data[$data_key][$key] = ( !empty( $value ) ) ? sanitize_text_field( $value ) : '';
					}
