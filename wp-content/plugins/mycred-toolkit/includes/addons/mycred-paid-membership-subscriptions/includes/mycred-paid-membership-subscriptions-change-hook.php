...
/**
 * Change Hook for Paid Membership Subscriptions plugin
 * @since 1.0
 * @version 1.0
 */
if (!class_exists('MyCred_PMS_Change_Subscription_Hook')):
    class MyCred_PMS_Change_Subscription_Hook extends myCRED_Hook
    {
        ...
        function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY)
        {
            ...
            parent::__construct(array(
                'id' => 'pms_change_subscription',
                'defaults' => array(
                    'creds' => 10,
                    'log' => __('%plural% for subscription change', 'myCred_pms'),
                    'limit' => '0/x',
        ...
        public function run()
        {
            add_action('pms_payment_update', array($this, 'mycred_pms_change_subscription_paid'), 10, 3);
            add_action('pms_after_checkout_is_processed', array($this, 'mycred_pms_change_subscription_free'), 10, 2);
            add_action('pms_manual_subscription_change_plan', array($this, 'mycred_pms_change_subscription_manual'), 10, 4);
        }
        ...
        /**
         * Award points for Change on Subscription (Free / No Payment)
         */
        public function mycred_pms_change_subscription_free($subscription, $form_location)
        {

            // Only care about change/upgrade/downgrade
            if (!in_array($form_location, array('change_subscription', 'upgrade_subscription', 'downgrade_subscription'))) {
                return;
            }
            ...
            // Check if a payment exists for this subscription change
            // If a payment exists, the 'pms_payment_update' hook will handle it.
            if (function_exists('pms_get_payments')) {
                ...
                if (!empty($payments)) {
                    $payment = $payments[0];
                    $payment_types = array('subscription_upgrade_payment', 'subscription_downgrade_payment');

                    // If the latest payment is one of these types AND is recent, assume payment hook handles it
                    if (in_array($payment->type, $payment_types)) {

                        $payment_time = strtotime($payment->date);
                        $current_time = time(); // Use UTC timestamp to match PMS (which uses date('Y-m-d H:i:s') in UTC)

                        // If payment was created less than 5 minutes ago, we assume it belongs to this transaction
                        // and let the payment hook handle it.
            ...
            // Award points for Free Change
            $user_id = $subscription->user_id;
            $subscription_plan_id = $subscription->subscription_plan_id;

            $this->award_points($user_id, $subscription_plan_id);
        }

        /**
         * Award points for Manual Subscription Change
         * Fires when admin completes the payment
         */
        public function mycred_pms_change_subscription_manual($old_subscription, $subscription_plan, $payment_id, $context)
        {
        ...
        /**
         * Award points for Change on Subscription (Paid)
         */
        public function mycred_pms_change_subscription_paid($payment_id, $new_data, $old_data)
        {
            ...
            // If this is manual gateway, skip here and let 'pms_manual_subscription_change_plan' handle it
            // to avoid issues with payment types or double awarding.
            if (isset($payment->payment_gateway) && $payment->payment_gateway == 'manual') {
                return;
            }

            // Check if this is a change payment (Upgrade or Downgrade)
            $allowed_types = array('subscription_upgrade_payment', 'subscription_downgrade_payment');
            if (!isset($payment->type) || !in_array($payment->type, $allowed_types)) {
                return;
            }
         ...
         * Shared award logic
         */
        public function award_points($user_id, $subscription_plan_id)
        {
            ...
            $prefs = $this->prefs;
            $reference = 'pms_change_subscription';
        ...
        /**
         * Preference for PMS Change Hook
         * @since 1.0
         * @version 1.0
         */
        public function preferences()
        {
            ...
            <div class="hook-instance">
                <h3><?php esc_html_e('Change any subscription', 'myCred_pms'); ?></h3>
                <div class="row">
            ...
            ?>
            <div class="hook-instance" id="specific-hook-pms-change">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="hook-title">
                            <h3><?php esc_html_e('Change specific subscription', 'myCred_pms'); ?></h3>
                        </div>
                    </div>
                </div>
                ...
                foreach ($specific_data as $hook_idx => $label) {
                    ?>
                    <div class="pms_change_specific_row">
                        <div class="row">
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    <label
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control mycred-pms-change-options"
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 field_wrapper">
                                <div class="form-group textright">
                                    <button class="button button-small mycred-add-specific-pms-change-hook add_button" type="button">Add
                                        More
                                    </button>
                                    <button class="button button-small mycred-remove-specific-pms-change-hook" type="button">Remove
                                    </button>
                                </div>
                            </div>
                        </div>
