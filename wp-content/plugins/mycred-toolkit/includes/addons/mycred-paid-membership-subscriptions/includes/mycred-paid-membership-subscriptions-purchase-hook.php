...
/**
 * Hook for Paid Membership Subscriptions plugin
 * @since 1.0
 * @version 1.0
 */
if (!class_exists('MyCred_PMS_Purchase_Subscription_Hook')):
    class MyCred_PMS_Purchase_Subscription_Hook extends myCRED_Hook
    {
        ...
        function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY)
        {
            ...
            parent::__construct(array(
                'id' => 'pms_purchase_subscription',
                'defaults' => array(
                    'creds' => 10,
                    'log' => __('%plural% for purchasing subscription', 'myCred_pms'),
                    'limit' => '0/x',
        ...
        public function run()
        {
            add_action('pms_member_subscription_insert', array($this, 'mycred_pms_purchase_subscription'), 10, 2);
            add_action('pms_member_subscription_update', array($this, 'mycred_pms_purchase_subscription'), 10, 3);
        }
        ...
        /**
         * Award points for Purchasing Subscription
         */
        public function mycred_pms_purchase_subscription($id, $data, $old_data = null)
        {
            // Determine Status
            ...
            if (!empty($old_data)) {
                ...
                // We only award if status becomes active
                if ($new_status != 'active') {
                    return;
                }
                // Prevent duplicate awarding if it was already active
                if ($old_status == 'active') {
                    return;
                }

                // Populate missing IDs from old_data if needed (updates might not contain them)
                if (!isset($data['subscription_plan_id']) && isset($old_data['subscription_plan_id'])) {
                    $data['subscription_plan_id'] = $old_data['subscription_plan_id'];
                }
                if (!isset($data['user_id']) && isset($old_data['user_id'])) {
                    $data['user_id'] = $old_data['user_id'];
                }

            }
            // Check Context: Insert
            else {
                if ($new_status != 'active') {
                    return;
                }
            ...
            $prefs = $this->prefs;
            $reference = 'pms_purchase_subscription';
        ...
        public function preferences()
        {
            ...
            <div class="hook-instance">
                <h3><?php esc_html_e('Purchase any subscription', 'myCred_pms'); ?></h3>
                <div class="row">
            ...
            ?>
            <div class="hook-instance" id="specific-hook-pms">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="hook-title">
                            <h3><?php esc_html_e('Purchase specific subscription', 'myCred_pms'); ?></h3>
                        </div>
                    </div>
                </div>
                ...
                foreach ($specific_data as $hook_idx => $label) {
                    ?>
                    <div class="pms_subscription_specific_row">
                        <div class="row">
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    <label
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control mycred-pms-subscription-options"
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 field_wrapper">
                                <div class="form-group textright">
                                    <button class="button button-small mycred-add-specific-pms-subscription-hook add_button"
                                        type="button">Add
                                        More
                                    </button>
                                    <button class="button button-small mycred-remove-specific-pms-subscription-hook"
                                        type="button">Remove
                                    </button>
                                </div>
                            </div>
