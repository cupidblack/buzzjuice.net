...
/**
 * Hook for Form Maker form submission
 * 
 * 
 */
 if ( ! class_exists( 'myCRED_Formmaker_Submit_Form_Hook' ) ) :
 	class myCRED_Formmaker_Submit_Form_Hook extends myCRED_Hook {
        ...
        function __construct( $hook_prefs, $type = 'mycred_default' ) {
            ...
            parent::__construct( array(
                'id' => 'formmaker_successful_submit_form',
                'defaults' => array( 
                    'creds'         => 0,
        ...
        public function run() {
            ...
            if ( is_user_logged_in() ) {
                ...
                add_action( 'fm_addon_frontend_init', array( $this, 'mycred_formmaker_submission' ), 10, 1 );
            }

        }

        public function mycred_formmaker_submission($frontend_params) {

            // Login is required
            if ( ! is_user_logged_in() ) return;
            
            $user_id = get_current_user_id();
            $form_id = $frontend_params['form_id']; 
            $prefs   = $this->prefs;

            if ( $this->core->has_entry( 'formmaker_successful_submit_form', $form_id, $user_id) ) return;

                // Execute
                $this->core->add_creds(
                    'formmaker_successful_submit_form',
                    $user_id,
                    $this->prefs['creds'],
                    $this->prefs['log'],
                    $form_id,
