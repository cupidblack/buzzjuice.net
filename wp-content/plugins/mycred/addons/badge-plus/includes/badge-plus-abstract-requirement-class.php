...
/**
 * myCRED_Badge_Requirement class
 * @since 1.0.0
 * @version 1.0.0
 */
if ( ! class_exists( 'myCRED_Badge_Requirement' ) ) :
	abstract class myCRED_Badge_Requirement {
		...
		abstract protected function has_met( $user_id, $requirement, $badge_id = 0 );
		...
		public function time_filter( $limit, $limit_by ) {
			...
			if ( $limit_by == 'day' ) {
				...
				$time_filter = ' AND time BETWEEN ' . strtotime( $limit_amount . ' day midnight', $now ) . ' AND ' . $now;
			...
			elseif( $limit_by == 'week' ) {
				...
				$time_filter = ' AND time BETWEEN ' . strtotime( $week, $now ) . ' AND ' . $now;

			}
			elseif( $limit_by == 'month' ) {

				$time_filter = ' AND time BETWEEN ' . strtotime( date( 'Y-m-01', $now ) . ' ' . $limit_amount . ' month' ) . ' AND ' . $now;

			}
			elseif( $limit_by == 'year' ) {

				$time_filter = ' AND time BETWEEN ' . strtotime( date( 'Y-01-01', $now ) . ' ' . $limit_amount . ' year' ) . ' AND ' . $now;
