...
/**
 * Hook for Presto Player - Video Percent
 * Awards points when user watches a specified percentage of a video
 * @since 1.0
 * @version 1.0
 */
if (!class_exists('myCRED_Presto_Player_Percent_Hook')):
    class myCRED_Presto_Player_Percent_Hook extends myCRED_Hook
    {
        ...
        function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY)
        {
            ...
            parent::__construct(array(
                'id' => 'presto_player_video_percent',
                'defaults' => array(
                    'creds' => 1,
                    'percent' => 50,
                    'log' => __('%plural% for watching video to percentage', 'mycred-toolkit'),
                    'limit' => '0/x',
        ...
        public function run()
        {
            ...
            add_action('presto_player_progress', array($this, 'mycred_presto_player_percent_progress'), 10, 3);
        ...
        /**
         * Award points for Video Percent
         */
        public function mycred_presto_player_percent_progress($id, $percent, $visit_time)
        {
            ...
            $prefs = $this->prefs;
            $reference = 'presto_player_video_percent';
            $video_id = absint($id);
            ...
            if ($specific_enabled && !empty($prefs['specific_videos']['select_option']) && $post_id) {
                ...
                if ($hook_index !== false) {
                    // Video is in specific list - check specific percent threshold

                    // Get the configured percent for this specific video
                    $required_percent = isset($prefs['specific_videos']['percent'][$hook_index])
                        ? absint($prefs['specific_videos']['percent'][$hook_index])
                        : 50;

                    // Check if current progress meets the required percent
            ...
            // Get the configured general percent threshold
            $general_percent = isset($prefs['percent']) ? absint($prefs['percent']) : 50;

            // Check if current progress meets the required percent
            if ($percent < $general_percent) {
                return;
            }
         ...
         */
        public function mycred_presto_player_percent_arrange_data($specific_hook_data)
        {
            $hook_data = array();
            if (isset($specific_hook_data['creds']) && is_array($specific_hook_data['creds'])) {
                foreach ($specific_hook_data['creds'] as $key => $value) {
                    ...
                    $hook_data[$key]['percent'] = isset($specific_hook_data['percent'][$key]) ? $specific_hook_data['percent'][$key] : 50;
        ...
        /**
         * Preference for Presto Player Percent Hook
         * @since 1.0
         * @version 1.0
         */
        public function preferences()
        {
            ...
            <!-- General Video Percent Rewards Starts -->
            <div class="hook-instance">
                <h3><?php esc_html_e('Watch any video to percent', 'mycred-toolkit'); ?></h3>
                <div class="row">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                        <div class="form-group">
                            <label
                                for="<?php echo $this->field_id('percent'); ?>"><?php esc_html_e('Percent', 'mycred-toolkit'); ?></label>
                            <input type="number" name="<?php echo $this->field_name('percent'); ?>"
                                id="<?php echo $this->field_id('percent'); ?>"
                                value="<?php echo isset($prefs['percent']) ? absint($prefs['percent']) : 50; ?>"
            ...
            <!-- General Video Percent Rewards Ends -->

            <!-- Specific Video Percent Rewards Starts -->
            <?php
            $specific_data = array(
                array(
                    'creds' => 0,
                    'percent' => 50,
                    'log' => __('%plural% for watching video to %percent%%', 'mycred-toolkit'),
                    'select_option' => 0
                ),
            );

            if (!empty($prefs['specific_videos']['creds']) && count($prefs['specific_videos']['creds']) > 0) {
                $specific_data = $this->mycred_presto_player_percent_arrange_data($prefs['specific_videos']);
            }

            ?>
            <div class="hook-instance" id="specific-hook-presto-player-percent">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="hook-title">
                            <h3><?php esc_html_e('Watch specific video to percent', 'mycred-toolkit'); ?></h3>
                        </div>
                    </div>
                </div>
                ...
                foreach ($specific_data as $hook_idx => $label) {
                    ?>
                    <div class="presto_player_percent_specific_row">
                        <div class="row">
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    <label
                            ...
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    <label><?php esc_html_e('Percent', 'mycred-toolkit'); ?></label>
                                    <input type="number"
                                        name="<?php echo $this->specific_field_name(array('specific_videos' => 'percent')); ?>"
                                        value="<?php echo isset($label['percent']) ? absint($label['percent']) : 50; ?>"
                                        class="form-control" min="1" max="100" />
                                </div>
                            </div>
                            ...
                            <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control mycred-presto-player-percent-options"
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 field_wrapper">
                                <div class="form-group textright">
                                    <button class="button button-small mycred-add-specific-presto-player-percent-hook add_button"
                                        type="button">Add More</button>
                                    <button class="button button-small mycred-remove-specific-presto-player-percent-hook"
            ...
            <!-- Specific Video Percent Rewards Ends -->
        ...
        function sanitise_preferences($data)
        {
            ...
            $data['percent'] = (!empty($data['percent'])) ? absint($data['percent']) : 50;
            ...
            if (isset($data['specific_videos'])) {
                foreach ($data['specific_videos']['creds'] as $key => $value) {
                    ...
                    $percent_val = isset($data['specific_videos']['percent'][$key]) ? $data['specific_videos']['percent'][$key] : 50;
                    $data['specific_videos']['percent'][$key] = absint($percent_val);
