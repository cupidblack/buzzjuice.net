...
function Learndash_Quiz_Failed_myCRED_Hook( $installed ) {
	...
	$installed['hook_quiz_failed_learndash'] = array(
		'title' => __('Failing at Quiz (LearnDash ) ', 'mycred-toolkit'),
		...
		class myCRED_Hook_Learndash_Quiz_Failed extends myCRED_Hook {
			...
			function __construct( $hook_prefs, $type = 'mycred_default' ) {
				parent::__construct(array(
					...
					'defaults' => array(
						'creds' => 0,
						'log' => __('%plural% for Failing at Quiz', 'mycred-toolkit'),
						'limit' => '0/x',
				...
				if (  isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['quiz_failed']['quiz_failed_select_quiz'] ) && in_array( $quiz_data['quiz']->ID, $prefs['quiz_failed']['quiz_failed_select_quiz'] )  ) {
				   ...
				   $hook_index = array_search( $quiz_data['quiz']->ID, $prefs['quiz_failed']['quiz_failed_select_quiz'] );
					...
					if (  
						...
						if ( $this->core->has_entry( 'learndash_quiz_failed', $quiz_data['quiz']->ID, $current_user->ID) ) {
return;
						}
			...
			public function preferences() {
				...
				?>
				 ...
				 $failed_quiz_data = array(
					 array(
						 'creds' => 0,
						 'log' => __('%plural% for Failing at Specific Quiz', 'mycred-toolkit'),
						 'limit' => '0/x',
						 'select_option' => 0,
				...
				<div class="quiz_failed_custom_hook_class">
					<div class="row">
									...
									<div class="form-group">
										...
										if ($label['select_option'] == 'quiz') {
											...
											$quiz_id = $quiz_name->ID;
												...
												echo '<option class="select-value" value="' . esc_attr( $quiz_name->ID ) . '" ' . ( $quiz_name->ID == $label['quiz_failed_select_quiz'] ? ' selected' : '' ) . ' >' . esc_attr( $quiz_name->post_title ) . '</option>';
											}
											...
											foreach ($tags as $tag) {
												...
												$fail_quiz_field = $this->quiz_name();
												 ...
												 echo '<option class="select-value" value="' . esc_attr( $tag->term_id ) . '" ' . ( $tag->term_id == $label['quiz_failed_select_quiz'] ? ' selected' : '' ) . ' >' . esc_attr ( $tag->name ) . '</option>';
						...
						<div class="col-lg-3 col-md-4 col-sm-12 col-xs-12">
							<div class="form-group">
								<label for="<?php echo $this->field_id(array( 'quiz_failed' => 'log' )); ?>"><?php esc_html_e('Log Template', 'mycred-toolkit'); ?></label>
