...
if (!class_exists('myCRED_FluentCart_Subscription_Activated_Hook')) :
    class myCRED_FluentCart_Subscription_Activated_Hook extends myCRED_Hook
    {
        public function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY)
        {
            parent::__construct(array(
                'id' => 'fluentcart_subscription_activated',
                'defaults' => array(
                    'creds' => 1,
                    'log' => __('%plural% for activating a subscription in FluentCart', 'mycred-toolkit'),
                    'limit' => '0/x',
        ...
        public function run()
        {
            add_action('fluent_cart/subscription_activated', array($this, 'handle_subscription_activated'), 10, 1);
        }
        ...
        public function handle_subscription_activated($data)
        {
            ...
            $prefs = $this->prefs;
            $reference = 'fluentcart_subscription_activated';
            ...
            for ($i = 1; $i <= $quantity; $i++) {
                $entry_id = $subscription_id . ':' . $product_id . ':' . $i;
                $this->award_subscription_activation_points($user_id, $product_id, $entry_id, $reference, $prefs, $specific_enabled, $specific_selected);
            }
        }

        private function award_subscription_activation_points($user_id, $product_id, $entry_id, $reference, $prefs, $specific_enabled, $specific_selected)
        {
        ...
        public function preferences()
        {
            ...
            <div class="hook-instance">
                <h3><?php esc_html_e('Activated any subscription', 'mycred-toolkit'); ?></h3>
                <div class="row">
            ...
            $specific_data = array(
                array(
                    'creds' => 0,
                    'log' => __('%plural% for activating a subscription for a specific product in FluentCart', 'mycred-toolkit'),
                    'select_option' => 0,
                ),
            );
            ...
            ?>
            <div class="hook-instance" id="specific-hook-fluentcart-subscription-products">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="hook-title">
                            <h3><?php esc_html_e('Activated subscription for specific product', 'mycred-toolkit'); ?></h3>
                        </div>
                    </div>
                </div>
