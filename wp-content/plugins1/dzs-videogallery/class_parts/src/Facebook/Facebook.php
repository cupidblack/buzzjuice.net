...
class Facebook
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!$config['app_id']) {
            throw new FacebookSDKException('Required "app_id" key not supplied in configs and could not find fallback environment variable "' . static::APP_ID_ENV_NAME . '"');
        }
        if (!$config['app_secret']) {
            throw new FacebookSDKException('Required "app_secret" key not supplied in configs and could not find fallback environment variable "' . static::APP_SECRET_ENV_NAME . '"');
        }
