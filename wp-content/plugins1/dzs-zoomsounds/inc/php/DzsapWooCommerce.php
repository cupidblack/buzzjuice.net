...
class DzsapWooCommerce {
  ...
  function wc_generate_player($id, $pargs = array()) {
    ...
    $player_position = isset($dzsap->mainoptions['wc_loop_player_position']) ? $dzsap->mainoptions['wc_loop_player_position'] : '';

    $extra_classes = isset($margs['extra_classes']) ? $margs['extra_classes'] : '';

    if (strpos($extra_classes, 'from-wc_single') !== false) {
      $player_position = isset($dzsap->mainoptions['wc_single_player_position']) ? $dzsap->mainoptions['wc_single_player_position'] : $player_position;
    }

    $mp3_meta = get_post_meta($post->ID, DZSAP_META_WOOCOMMERCE_ZOOMSOUNDS_MP3, true);
    if (!$mp3_meta) {
      // nothing to generate
      return false;
    }

    // We will generate the player now; mark as generated so duplicates are avoided.
    $this->has_generated_product_player = true;

    // increment player index
    if (is_object($dzsap)) {
      if (!isset($dzsap->sliders__player_index)) {
        $dzsap->sliders__player_index = 0;
      }
    ...
    $argsForShortcode = array(
      'mp3'    => $src,
      'config' => isset($dzsap->mainoptions['wc_single_product_player']) ? $dzsap->mainoptions['wc_single_product_player'] : '',
    );
    ...
    $argsForShortcode['config'] = isset($dzsap->mainoptions['wc_single_product_player']) ? $dzsap->mainoptions['wc_single_product_player'] : $argsForShortcode['config'];
    $argsForShortcode['extra_classes'] = $extra_classes;

    if ($this->wc_called_loop_from == 'loop') {
      $argsForShortcode['config'] = isset($dzsap->mainoptions['wc_loop_product_player']) ? $dzsap->mainoptions['wc_loop_product_player'] : $argsForShortcode['config'];
    }

    if (!empty($dzsap->mainoptions['wc_product_play_in_footer']) && $dzsap->mainoptions['wc_product_play_in_footer'] == 'on') {
      $argsForShortcode['faketarget'] = '.' . DZSAP_VIEW_STICKY_PLAYER_ID;
    }

    if ($player_position == 'overlay') {
      // append prevent-bubble but keep previously provided classes
      $argsForShortcode['extra_classes'] = trim($argsForShortcode['extra_classes'] . ' prevent-bubble');
    }

    if (is_string($argsForShortcode['source']) && strpos($argsForShortcode['source'], 'https://soundcloud.com') !== false) {
    ...
    $author_name = '';
    if ($recent_author && isset($recent_author->display_name)) {
      $author_name = $recent_author->display_name;
    } else {
      $author_name = '';
    }

    $argsForShortcode['artistname'] = $author_name;
    $argsForShortcode['menu_artistname'] = $author_name;

    if (isset($margs['call_from']) && $margs['call_from'] == 'wc_loop') {
    ...
    $tax = defined('DZSAP_TAXONOMY_NAME_SLIDERS') ? DZSAP_TAXONOMY_NAME_SLIDERS : '';

    $is_playlist = false;

    $playlist_slug = (defined('DZSAP_WOOCOMMERCE_PLAYLIST_IN_PRODUCT_PREFIX') ? DZSAP_WOOCOMMERCE_PLAYLIST_IN_PRODUCT_PREFIX : '') . $argsForShortcode['product_id'];

    $reference_term = ($tax) ? get_term_by('slug', $playlist_slug, $tax) : false;
    ...
    if ($is_playlist) {
      ...
      if (isset($margs['call_from']) && $margs['call_from'] == 'handle_woocommerce_single_product_summary' && !empty($dzsap->mainoptions['wc_single_product_player_shortcode'])) {
...
function dzsap_woo_product_write_panel_tab() {
  echo "<li class=\"product_tabs_lite_tab\"><a href=\"#woocommerce_dzsap_tab\"><span>" . esc_html__('ZoomSounds', DZSAP_ID) . "</span></a></li>";
}
...
function dzsap_woo_product_write_panel() {
  ...
  woocommerce_wp_text_input(array('id' => $lab, 'class' => 'upload-target-prev', 'label' => esc_html__('Preview Track', DZSAP_ID), 'description' => ('<button class="button-secondary action dzs-wordpress-uploader ">'.esc_html__('Upload', DZSAP_ID).'</button>'), 'value' => get_post_meta($post->ID, $lab, true)));
  ...
  woocommerce_wp_checkbox(array('id' => $lab, 'class' => ' ', 'label' => esc_html__('Use for download', DZSAP_ID), 'value' => $val));
  ...
  echo '<div class="woocommerce_dzsap_tab-setting">';
  woocommerce_wp_text_input(array('id' => $lab, 'class' => 'sample-time-end-feeder', 'label' => esc_html__('Sample Time End', DZSAP_ID), 'description' => esc_html__('If this is a sample ( you are not showing the full track, you can input a end time here ), leave 0 if not', DZSAP_ID), 'value' => $val));
  ...
  echo '<div class="woocommerce_dzsap_tab-setting">';
  woocommerce_wp_text_input(array('id' => $lab, 'class' => 'sample-time-total-feeder', 'label' => esc_html__('Sample Time Total', DZSAP_ID), 'description' => esc_html__('The total track duration  ( in seconds ) ', DZSAP_ID), 'value' => $val));
  ...
  echo '<div class="woocommerce_dzsap_tab-setting">';
  woocommerce_wp_text_input(array('id' => $lab, 'class' => '', 'label' => esc_html__('Subtitle', DZSAP_ID), 'description' => esc_html__('The subtitle for some grid styles', DZSAP_ID), 'value' => $val));
  ...
  echo '<div class="woocommerce_dzsap_tab-setting">';
  woocommerce_wp_text_input(array('id' => $lab, 'class' => '', 'label' => esc_html__('Custom Link on Buy', DZSAP_ID), 'description' => esc_html__('Custom link on Buy button click', DZSAP_ID), 'value' => $val));
