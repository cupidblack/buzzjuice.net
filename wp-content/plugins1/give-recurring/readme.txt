...
== Changelog ==
= 2.5.0: September 13th, 2023 =
* Feature: Added Stripe Payment Element subscription module
* Feature: Enabled recurring by default in new forms created using the visual form builder
* Feature: Added Test Gateway subscription module 
* Feature: Added PayPal Standard Gateway subscription module 
* Feature: Used the new SubscriptionSynced command class on the Test Gateway and implement a way to display the synced data returned by it in the U
* Feature: Added PayPal Commerce subscription module 
* Fix: Created renewals for Authorize and Square gateways with the proper status 

= 2.4.3: August 7th, 2023 =
* Fix: Update missing transaction ID for renewals already recorded.

= 2.4.3: June 13th, 2023 =
* Fix: Action scheduler events are not shown as failed when they run as expected.

= 2.4.2: May 23rd, 2023 =
* Refactor: Remove Recurring add-ons tab and page from Settings

= 2.4.1: May 11th, 2023 =
* Fix: Donors can cancel subscriptions from the donor dashboard without error.
* Fix: Square plan names are invoiced as the donation form title.
* Fix: When the email access option is disabled, donations made by users who are not logged in will be finished without throwing an error.

= 2.4.0: April 6th, 2023 =
* Enhancement: A number of improvements for compatibility with the upcoming Donation Form Builder feature plugin release
* Fix: Cleaned up some PHP notices thrown by the PayPal Donations webhooks
* Fix: In the case that canceling a subscription fails, the subscription status is no longer marked as canceled
* Fix: Improved PHP 8.0+ compatibility

= 2.3.1: March 22nd, 2023 =
* Fix: Donor's email and order description are now recorded properly with Authorize.Net

= 2.3.0: March 1st, 2023 =
* New: Now supports Square recurring donations!
* Fix: Added additional security checks to subscription link generation
* Fix: Fixed a typo, no more confusion

= 2.2.0: January 26th, 2023 =
...
= 2.1.2: November 22nd, 2022 =
* Fix: Resolved an issue preventing Stripe + Plaid recurring donations from working.
* Fix: Quarterly intervals now work properly for Stripe recurring donations.

= 2.1.1: November 17th, 2022 =
* Fix: Syncing subscription works properly with PayPal Standard, PayPal Pro, and Authorize.net
* Fix: Stripe SEPA donations are no longer immediately set to complete until the webhook is received
* Fix: Updating payment information on a subscription no works properly on the legacy manage subscriptions page

= 2.1.0: October 31st, 2022 =
* New: Recurring now supports PayFast!!
* Enhancement: Under the hood improvement to how subscriptions and donations are connected - small but mighty
* Fix: Improved compatibility with upcoming WordPress 6.1 release
* Fix: Exporting upcoming renewals once again works properly
* Fix: Resolved an issue causing an error when using Recurring with the Manual Donations add-on

= 2.0.0: September 13th, 2022 =
* Enhancement: Complete rebuild of all gateways to use the new Gateway API
* Fix: Resolves a niche issue for Stripe accounts containing existing users
* Fix: Links for viewing the receipt in the legacy Donor Subscription shortcode now work if the Donor Dashboard is enabled
* Fix: Correct Authorize.net status problem which broken {subscription_frequency} email tag

= 1.15.1: July 11th, 2022 =
* Fix: Make sure the anonymous donation option works as expected 
