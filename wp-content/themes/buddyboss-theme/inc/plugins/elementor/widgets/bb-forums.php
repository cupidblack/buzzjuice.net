												...
												<?php if ($settings['switch_meta']) : ?>
													<div class="bb-forums__ww">
														<span class="bs-replied">
															<?php
															// Fix: ensure bbp_author_link() return is handled (avoid passing null to str_replace)
															$author_link_raw = bbp_author_link( array( 'post_id' => bbp_get_topic_last_active_id(), 'size' => 1 ) );
															if ( is_null( $author_link_raw ) ) {
																$author_link_raw = '';
															}
															$bbp_author_link = str_replace( '&nbsp;', '', (string) $author_link_raw );
															?>
