...
class Users {
            ...
            if ($user) {
                ...
                if ($data) {
                    ...
                    if (isEndPointRequest()) {
                        ...
                        unset($response['data']['user_info']->avatar);
                        $response['data']['user_info']->avatar = $avatar;
                    }
                    return json($response, 200);
                } else {
                    return json(array(
    ...
    public function login($username = '', $password = '') {
        ...
        if ($this->isPasswordVerifyed($username, $password)) {
            ...
            if ($user) {
                ...
                $data = $this->createSession($user['id'], $user);
                if ($data) {
                    $profile =  $this->get_user_profile($user['id'],array('avatar','web_token','start_up','active','web_token_created_at','verified','admin'));
                    $response = array(
                        'message' => __('Login successfully, Please wait..'),
                        'code' => 200,
                        'userProfile' => $profile,
                    ...
                    if (isEndPointRequest()) {
                        ...
                        unset($response['data']['user_info']->password);
                        unset($response['data']['user_info']->web_device);
                        $avatar = GetMedia($profile->avatar);
                        unset($response['data']['user_info']->avatar);
                        $response['data']['user_info']->avatar = $avatar;
                    }
                    return json($response, 200);
    ...
    public function register($data = array()) {
        ...
        $user['avatar']               = (ISSET($data['avatar']) ? Secure($data['avatar']) : $config->userDefaultAvatar);
            ...
            foreach ($_fetch as $key){
                if($key == 'data') {
                    if (isEndPointRequest()) {
                        ...
                        $result->avatar = GetMedia($result->avatar, false);
    ...
    public function ImportImageFromLogin($media, $amazon = 0) {
        ...
        $safe_dir = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.jpg';
        $safe_dir2 = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar_full.jpg';
    ...
    public function delete_user($user_id){
        ...
        foreach ($_media_files as $key => $value){
            ...
            $media_uploaded_files[] = str_replace('_full.','_avatar.',$value->file);
        }
    ...
    public function list_visits(){
        ...
        $visits = $db->objectBuilder()
            ...
            ->orderBy('v.created_at', 'DESC')
            ->get('views v', $limit, array('u.id','u.username','u.avatar','u.country','u.first_name','u.last_name','u.location','u.birthday','u.language','u.relationship','u.height','u.body','u.smoke','u.ethnicity','u.pets','v.created_at'));
        foreach ($visits as $key => $value){
            $visits[$key]->avatar = GetMedia(trim($visits[$key]->avatar));
        }
        return json(array(
            'data' => $visits,
            'code' => 200
    ...
    public function matches(){
        ...
        if (empty($_POST['access_token'])) {
            ...
            foreach ($match_users as $key => $value){
                ...
                $match_users[$key]->avatar = GetMedia($match_users[$key]->avatar);
                ...
                if ($mediafiles) {
                    ...
                    foreach ($mediafiles as $mediafile) {
                        if($mediafile['file']) {
                            ...
                            $match_users[$key]->mediafiles[$mediafilesid]['avatar'] = GetMedia(str_replace('_full.', '_avatar.', $mediafile['file']), false);
                            $mediafilesid++;
                        }
                    }
                }else{
    ...
    public function random_users(){
        ...
        if (empty($_POST['access_token'])) {
            ...
            $limit = ( isset($_POST['limit']) && is_numeric($_POST['limit']) && (int)$_POST['limit'] > 0 ) ? (int)$_POST['limit'] : 20;
            $offset  = ( isset($_POST['offset']) && is_numeric($_POST['offset']) && (int)$_POST['offset'] > 0 ) ? (int)$_POST['offset'] : 0;
            $sql = 'SELECT `id`,`online`,`lastseen`,`username`,`avatar`,`country`,`first_name`,`last_name`,`location`,`birthday`,`language`,`relationship`,`height`,`body`,`smoke`,`ethnicity`,`pets`,`gender` FROM `users` ';
            $sql .= ' WHERE  ';
            ...
            foreach ($random_users as $key => $value){
                if (isEndPointRequest()) {
                    ...
                    $value->avatar = GetMedia($value->avatar);
                    ...
                    if( isset( $result->id ) ) {
                        ...
                        if ($mediafiles) {
                            ...
                            foreach ($mediafiles as $mediafile) {
                                ...
                                $result->mediafiles[$mediafilesid]['avatar'] = GetMedia(str_replace('_full.', '_avatar.', $mediafile['file']), false);
                                $mediafilesid++;
                            }
                        }
                    }
    ...
    public function update_avatar(){
        ...
        if (empty($_FILES['avatar']) || empty($_POST['access_token'])) {
            return json(array(
                'code' => 400,
                'errors' => array(
                    'error_id' => '23',
            ...
            if (move_uploaded_file($_FILES['avatar']['tmp_name'], $filename)) {
                $thumbfile = '../upload' . $_DS . 'photos' . $_DS .  date('Y') . $_DS . date('m') . $_DS . $key . '_avatar.' . $ext;
                ...
                $files_name = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.' . $ext;
                $saved = $db->where('id', $_userid)->update('users',array('avatar'=>$files_name));
                if($saved){
                    return json(array(
            ...
            if (!empty($social_id)) {
                ...
                if (isset($this->isEmailExists($social_email)['email'])) {
                    ...
                    if( !empty($userData['avatar']) ){
                        $_POST['avatar'] = $this->ImportImageFromLogin($userData['avatar'], 1);
                    }
    ...
    public function delete_media_file(){
        ...
        if (empty($_POST['id']) || empty($_POST['access_token'])) {
            ...
            $avatar_file = str_replace('_full.', '_avatar.', $url);
            DeleteFromToS3($url);
    ...
    public function upload_media_file() {
        ...
        if (empty($_FILES['avatar']) || empty($_POST['access_token'])) {
            ...
            $filename = $dir . $_DS . $key . '.' . $ext;
            if (move_uploaded_file($_FILES['avatar']['tmp_name'], $filename)) {
                $thumbfile = '../upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.' . $ext;
    ...
    public function get_pro(){
        ...
        if (empty($_POST['access_token']) || empty($_POST['limit'])) {
            ...
            $pro_users = $db->objectBuilder()
                ...
                ->get('users',$limit,array('id','username','avatar','active','is_pro'));
            ...
            if( !empty($_FILES['video_thumbnail']) ) {
                ...
                if (move_uploaded_file($_FILES['video_thumbnail']['tmp_name'], $photo_filename)) {
                    ...
                    if (is_file($photos_org_file)) {
                        ...
                        $update_data['file'] = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $photo_key . '.' . $photo_ext;
                        $update_data['private_file'] = ((int)Secure($_POST['privacy']) === 1) ? '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $photo_key . '.' . $photo_ext : '';
                    }
                }
            }
            if( !empty($_FILES['video']) ) {
                ...
                if (move_uploaded_file($_FILES['video']['tmp_name'], $video_filename)) {
                    ...
                    if (is_file($video_org_file)) {
                        ...
                        $update_data['video_file'] = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $video_key . '.' . $video_ext;
                    }
                }
            }
        ...
        if (empty($_FILES['video']) || empty($_FILES['video_thumbnail']) || empty($_POST['privacy']) || empty($_POST['access_token'])) {
            ...
            if (move_uploaded_file($_FILES['video_thumbnail'][ 'tmp_name' ], $photo_filename)) {
                ...
                if (is_file($photos_org_file)) {
                    ...
                    $insert_data['file'] = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $photo_key . '.' . $photo_ext;
                    $insert_data['private_file'] = ( (int)Secure($_POST['privacy']) === 1 ) ? '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $photo_key . '.' . $photo_ext : '';
                }
            }
            if (!file_exists($_UPLOAD . 'videos' . $_DS . date('Y'))) {
                mkdir($_UPLOAD . 'videos' . $_DS . date('Y'), 0777, true);
            ...
            if (move_uploaded_file($_FILES['video'][ 'tmp_name' ], $video_filename)) {
                ...
                if (is_file($video_org_file)) {
                    ...
                    $insert_data[ 'video_file' ] = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $video_key . '.' . $video_ext;
                }
            }
