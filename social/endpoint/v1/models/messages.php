...
class Messages {
    ...
    public function createNewConversation($receiver_id, $userid = null) {
        ...
        if (!empty($sid) && isset($sid['id']) && $sid['id'] > 0) {
            $data = [];
            $data['created_at'] = time();
            if ($config->message_request_system == 'on') {
                if (isset($sid['status']) && (int)$sid['status'] == 2) {
                    if (isset($sid['created_at']) && intval($sid['created_at']) < strtotime("-1 day")) {
                        $data['status'] = 0;
                        $db->where('sender_id', $uid)->where('receiver_id', (int)$receiver_id)->update('conversations', ['status' => 0]);
                        $db->where('sender_id', (int)$receiver_id)->where('receiver_id', $uid)->update('conversations', ['status' => 1]);
                        $isnew = true;
                    }
                }
            }
            ...
            $dat = [
                ...
                'created_at' => time(),
                'status' => ($config->message_request_system == 'on') ? 0 : 1,
            ];
            $db->insert('conversations', $dat);
            $isnew = true;
        }

        $rid = $db->where('sender_id', (int)$receiver_id)->where('receiver_id', $uid)->getOne('conversations', ['id']);
        if (!empty($rid) && isset($rid['id']) && $rid['id'] > 0) {
            $db->where('id', $rid['id'])->update('conversations', ['created_at' => time()]);
        } else {
            $dat2 = [
...
*/
    ...
    public function getUnreadMessages() {
        ...
        if (!$auth || !isset($auth->id)) {
            //error_log("Error: User is not authenticated in getUnreadMessages.");
            return 0; // Return 0 unread messages if the user is not authenticated
        }
    
        $blocked_user_array = (array_keys(BlokedUsers())) ? array_keys(BlokedUsers()) : array();
        $messages = $db->objectBuilder()
            ->where('`to`', $auth->id)
            ->where('`from`', $blocked_user_array, 'NOT IN')
            ->where('seen', 0)
    ...
    public function userData($uid){
        ...
        while ($row = @mysqli_fetch_array($msg, MYSQLI_ASSOC)) {
            ...
            $result->avatar = GetMedia($row['avatar']);
            ...
            if (isset($msg[0])) {
                ...
                if (count( (array)$user) > 0) {
                    ...
                    if (isEndPointRequest()) {
                        ...
                        $dt['user'] = userProfile( $value->receiver_id, array('id','avatar','username','first_name','last_name', 'lastseen'));
                        $dt['user']->avatar = GetMedia($dt['user']->avatar);
        ...
        if (empty($_FILES['media_file']) || empty($_POST['to_userid']) || $token == null) {
            ...
            foreach ($_FILES as $file) {
                ...
                if (move_uploaded_file($file['tmp_name'], $filename)) {
                    ...
                    $msg['from']       = $user_id;
                    $msg['to']         = $to_userid;
                    $msg['media']      = '../streams/upload/chat/' . date('Y') . '/' . date('m') . '/' . $key . '_m.' . $ext;
                    $msg['seen']       = 0;
                    ...
                    if ($saved) {
                        ...
                        $file                = GetMedia('../streams/upload/chat/' . date('Y') . '/' . date('m') . '/' . $key . '_m.' . $ext);
                    }
                } else {
                    $error = true;
                }
    ...
    public function get_chat_conversations() {
        ...
        if (empty($_POST['access_token']) || empty($_POST['to_userid'])) {
            ...
            foreach ($chat_messages as $key => $value) {
                ...
                $chat_messages[$key]['from_avatar'] = GetMedia($chat_messages[$key]['from_avatar']);
                $chat_messages[$key]['to_avatar']   = GetMedia($chat_messages[$key]['to_avatar']);
