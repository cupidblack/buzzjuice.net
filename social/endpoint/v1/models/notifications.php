...
class Notifications {
    ...
    public function getUnreadNotifications() {
        ...
        if (!$user || empty($user->id)) {
            // Log an error and return 0 if the user is not authenticated
            //error_log("Error: User is not authenticated in getUnreadNotifications.");
            return 0;
        }
        ...
        $notifications = $db->objectBuilder()
            ...
            ->where('seen', 0)
            ->where('`notifier_id` NOT IN (SELECT `block_userid` FROM `blocks` WHERE `user_id` = ' . $user_id . ')')
    ...
    public function get_notifications() {
        ...
        if (empty($_POST['access_token'])) {
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                $sql['avatar'] = GetMedia($fromuserresult->avatar, false);
                $audio_chat = $sql;
            }
            ...
            if (mysqli_num_rows($query2) > 0) {
                ...
                $sql2['avatar'] = GetMedia($fromuserresult2->avatar, false);
                $video_chat = $sql2;
            }
