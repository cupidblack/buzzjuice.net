...
function SendMessages() {
    ...
    $.get(window.ajax + 'useractions/avatar', {});
}
...
function logout() {
    ...
    // Optionally, clear specific cookies if needed
    document.cookie = 'JWT=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/; SameSite=None; Secure';

    // Destroy the user's session (if applicable, handled server-side)
    // You can make an AJAX call to the server to destroy the session
    fetch(window.site_url + '/streams/logout_session', { method: 'POST' })
        .then(() => {
            // Redirect to the logout URL
            window.location = 'https://buzzjuice.net/streams/logout/?social=home';
        })
        .catch(() => {
            // Fallback redirect if the session destruction fails
            window.location = 'https://buzzjuice.net/streams/logout/?social=home';
        });
}
...
function callback_load_more_search_users( result ) {
    ...
    if (result.status == 200) {
        ...
        if (result.html.length == 0) {
            ...
            if($('#avatars_item_container').length == 0){
                $('#section_match_users').html(`<div class="dt_home_match_user">
                <div class="valign-wrapper mtc_usr_avtr" id="avatars_item_container">
                        ${result.html_imgs}
                    </div>
                ...
                $('#avatars_item_container').html(result.html_imgs);
...
function callback_load_more_match_users( data ) {
    let avatars_container = $('#avatars_item_container');
    ...
    if( data.status == 200 ) {
        if( data.html !== '' ){
            ...
            avatars_container.append(data.html_imgs);
    ...
    //     let button = $('#btn_load_more_match_users');
    //     let avatars_container = $('#avatars_item_container');
    //     let avatars_item_templates = $('#match-users-avatars-item');
    //     let avatars_itemtemplateHtml = avatars_item_templates.html().trim();
    //     let avatars_itemdtemplateHtml = '';
    //     let avatars_itemlistHtml = '';
    //
    //     let container = $('#match_item_container');
    ...
    //                 }
    //                 avatars_itemdtemplateHtml = avatars_itemtemplateHtml.interpolate(data.list[key]);
    //                 dtemplateHtml = templateHtml.interpolate(data.list[key]);
    //                 avatars_itemlistHtml += avatars_itemdtemplateHtml;
    //                 listHtml += dtemplateHtml;
    //             }
    //         }
    //         avatars_container.append(avatars_itemlistHtml);
...
function callback_hot( data ){
    ...
    if( data.status == 200 ) {
        ...
        //     $('#btn_load_more_random_users').trigger('click');
        // }
        // if($('#avatars_item_container .usr_thumb').length < 2 ){
