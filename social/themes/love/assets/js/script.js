    ...
    $(function(){
            ...
            if (extn == "gif" || extn == "png" || extn == "jpg" || extn == "jpeg") {
                if (typeof(FileReader) != "undefined") {
                    ...
                    for (var i = 0; i < countFiles; i++) {
                        ...
                        formData.append("avatars"+i, $(this)[0].files[i],$(this)[0].files[i].value );
                    }
        ...
        $( document ).on( 'change', '#profileavatar_img', function(e){
            ...
            if(countFiles > 1) {
                ...
                var formData = new FormData();
                formData.append("avatars0", $(this)[0].files[0],$(this)[0].files[0].value );
                ...
                $.ajax({
                    ...
                    url: window.ajax + 'profile/upload_avatar',
                    type: "POST",
                    async: true,
                    data: formData,
                    cache: false,
                    ...
                    success: function(result) {
                        if( result.status == 200 ){
                            $.each( result.files, function(i) {
                                $.get( window.ajax + 'profile/set_avatar', { id: result.files[i] } );
        ...
        $( document ).on( 'change', '#admin_profileavatar_img', function(e){
            ...
            $.ajax({
                ...
                success: function(result) {
                    if( result.status == 200 ){
                        $.each( result.files, function(i) {
                            $.get( window.ajax + 'profile/set_user_avatar', { userid: user_id , id: result.files[i] } );
                            $('.avatar').find('img').attr( 'src', window.media_path + result.files[i] );
                        })
                        setTimeout(function() {
        ...
        */
        ...
        $( document ).on( 'click', 'button.like', function(e) {
            ...
            if($(this).hasClass('hot')){
                var execlude = [];
                $('#avatars_item_container .usr_thumb').each(function(i, obj) {
                    //console.log(i, $(obj).attr('data-id') );
                    execlude.push($(obj).attr('data-id'));
                });
                //console.log(execlude.join());
                var lastid = $('#avatars_item_container div:last-child').attr('data-id');
