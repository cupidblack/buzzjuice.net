	...
	<div class="row r_margin">
		...
		<div class="col l3 profile_menu">
			<div class="dt_left_sidebar dt_profile_side">
				<div class="avatar">
					<a class="inline" href="<?php echo $profile->avatar->full;?>" id="avatar_profile_img">
						<img src="<?php echo $profile->avatar->avatar;?>" alt="<?php echo $profile->full_name;?>" class="responsive-img" />
			...
			<div class="mtc_usrd_content dt_profile_about">
				<div class="row no_margin r_margin">
										...
										foreach ($gallery['visable'] as $key => $value) {
											if( !empty($value) && $value['is_video'] == "1" && $value['is_confirmed'] == "0" ){
												...
												if (!empty($value)) {
													...
													$is_avatar = 'false';
													if ($value['avatar'] == $profile->avatar->avatar) {
														$is_avatar = 'true';
													}
													$full = $value['full'];
													$avatar = $value['avatar'];
													...
													if ($value['is_private'] == 1 && $matched === false) {
														...
														$avatar = $value['private_file_avatar'];
													}
													if($config->review_media_files == '1' && $value['is_approved'] == 1){
														...
														if( $value['is_video'] == "1" ){
															...
															if ($value['is_private'] == 1 && $matched === false) {
																echo '<a class="inline" href="' . $value['full'] . '" data-fancybox="gallery" data-id="' . $value['id'] . '" data-private="' . $private . '" data-avatar="' . $is_avatar . '">';
															}else{
																echo '<a class="inline" href="#myVideo_'.$value['id'].'" data-fancybox="gallery" data-id="' . $value['id'] . '" data-video="' . $value['is_video'] . '" data-private="' . $private . '" data-avatar="' . $is_avatar . '">';
					...
					//                                          echo '    <source src="https://www.html5rocks.com/en/tutorials/video/basics/Chrome_ImF.ogv" type="video/ogg">';
														...
														echo '<img src="' . $avatar . '" alt="' . $profile->username . '">';
														echo '</a>';
														echo '</div>';
													} else {
														if($config->review_media_files == '0' && $value['is_approved'] == 1) {
															...
															if( $value['is_video'] == "1" ){
																...
																if ($value['is_private'] == 1 && $matched === false) {
																	echo '<a class="inline" href="' . $avatar . '" data-fancybox="gallery" data-id="' . $value['id'] . '" data-private="' . $private . '" data-avatar="' . $is_avatar . '">';
																}else{
																	echo '<a class="inline" href="#myVideo_'.$value['id'].'" data-fancybox="gallery" data-id="' . $value['id'] . '" data-video="' . $value['is_video'] . '" data-private="' . $private . '" data-avatar="' . $is_avatar . '">';
																	echo '<video width="800" height="550" controls id="myVideo_'.$value['id'].'" style="display:none;">';
																	echo '    <source src="'.$video_file.'" type="video/mp4">';
...
<div id="modal_gifts" class="modal modal-lg modal-fixed-footer">
    <div class="modal-content">
        ...
        <?php if ($user !== null && ($user->balance >= $config->cost_per_gift || isGenderFree($user->gender) === true)) { ?>
            <div id="gifts_container"></div>
        <?php } else { ?>
            <div id="buy_credits_gift" class="hide">
                <div class="credit_bln" style="margin-top: 130px;">
                    <div>
                        ...
                        <h2><?php echo __('Your') . ' ' . ucfirst($config->site_name) . ' ' . __('Credits balance'); ?></h2>
...
if( route(2) == 'opengift' && is_numeric(route(3)) ) {
    ...
    if ($gifts) {
        ...
        foreach ($gifts as $key => $value) {
            $gift_sender = userData($value->from, array('first_name', 'last_name', 'username', 'avatar'));
            ?>
...
?>
    <div id="story_approval" class="modal modal-fixed-footer" style="width: 30%;">
        ...
        <div class="modal-footer">
            ...
            <?php if ($user !== null && $user->id !== $profile->id) { ?>
                <a href="javascript:void(0);" id="approve_story" data-storyid="<?php echo route(3); ?>" data-story-userid="<?php echo $user->id; ?>" data-story-to-userid="<?php echo $profile->id; ?>" class="modal-close waves-effect waves-light btn-flat btn_primary white-text"><?php echo __( 'Approve story' ); ?></a>
            <?php } ?>
        </div>
    </div>
...
//    }
	...
	*/
	...
	if ($user !== null && Wo_IsFollowRequested($profile->id, (int) $user->id)) { ?>
		<div id="story_approval" class="modal" style="width: 30%;">
			<div class="modal-content">
				<div class="center">
					<svg version="1.1" xml:space="preserve" width="80" height="80" viewBox="0 0 682.66669 682.66669" xmlns="http://www.w3.org/2000/svg" xmlns:svg="http://www.w3.org/2000/svg">
						<!-- SVG content -->
					</svg>
				</div>
				<p></p>
