...
if (session_status() == PHP_SESSION_NONE) {
    SessionStart(); // starts session with correct params & SSO hydration
}

// Ensure QD config ($config) is available before using it
...
/*if ($config->wowonder_login == '1') {
    $login_url = $config->wowonder_domain_uri . '/oauth?app_id=' . $config->wowonder_app_ID;
...
if (IS_LOGGED === false) {
    $login_url = 'qd-sso-bridge.php';

    // Ensure no duplicate slashes in the final URL
    $redirect_url = rtrim($config->uri, '/') . '/' . ltrim($login_url, '/');
					...
					<div>
						<div class="user">
							<a href="<?php echo $site_url;?>/@<?php echo $value->username?>">
								<img src="<?php echo $value->avatar->full?>">
								<div class="padd">
					...
					<div class="xuser hide">
						<?php foreach ($siteUsers as $key => $user) { ?>
							<a href="<?php echo $site_url;?>/@<?php echo $user->username;?>" data-ajax="/@<?php echo $user->username;?>">
								<img src="<?php echo $user->avatar->avatar;?>" alt="<?php echo $user->full_name;?>" class="circle">
							</a>
						<?php } ?>
					</div>
				<?php }} ?>
								...
								<?php foreach ($stories as $key => $story){ ?>
									<div class="swiper-slide">
										<div class="dt_testimonial_slide">
											<div class="hdr">
												<div>
													<img src="<?php echo $story['user1Data']->avatar->avatar;?>" alt="<?php echo $story['user1Data']->full_name;?>" class="circle" />
													<img src="<?php echo $story['user2Data']->avatar->avatar;?>" alt="<?php echo $story['user2Data']->full_name;?>" class="circle" />
												</div>
