...
<!--									<button id="stripe_credit_btn" class="hide"></button>-->
    <?php if($config->bank_payment == '1'  && auth()->username == 'drenkaby'){?>
    ...
    <?php } ?>
    <?php if ($config->paystack_payment == "yes" && !empty($config->paystack_secret_key) && auth()->username == 'drenkaby') { ?>
	...
	<?php if ($config->qdw_payment == "yes" && !empty($config->qdw_api_key)) { ?>
    <!-- BlueCrownR&D - QDW-PGB Button for Credit Top-ups -->
		<button id="qdw-pgb-button" class="valign-wrapper btn btn-qdw-pgb-payment qdw-pgb" onclick="InitializeQDWPGB();">
