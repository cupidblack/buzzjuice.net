					...
					<div class="dt_plans">
						<p>
							...
							<label for="plan_1" class="plan_1">
								...
								<b><svg xmlns="http://www.w3.org/2000/svg" width="37" height="42" viewBox="0 0 13.658 15.175">
									<path 
										fill="#030C5E" 
										stroke="#030C5E" 
										stroke-width="12" 
...
<?php if ($config->paystack_payment == 'yes') { ?>
    ...
    function InitializeWalletPaystack() {
		...
		price = getPrice();
		$.post(window.ajax + 'qdw-pgb/initialize', {type:'credit',email:email,price:price}, function(data) {
			if (data.status == 200) {
				window.location.href = data.url;
			} else {
...
// BlueCrownR&D - QDW-PGB Function for Credit Top-ups
<?php if ($config->qdw_payment == 'yes') { ?>
    // BlueCrownR&D - QDW-PGB Function for Credit Top-ups
    function InitializeQDWPGB() {
        $('#qdw-pgb_btn').attr('onclick', 'InitializeWalletQDWPGB()');
        $('#qdw-pgb_wallet_modal').modal('open');
    }
    function InitializeWalletQDWPGB() {
        $('#qdw-pgb_btn').html('<span class="processing"><?php echo __('Please Wait');?></span>');
        $('#qdw-pgb_btn').attr('disabled', 'true');
        const price = getPrice();

        $.post(window.ajax + 'qdwpgb/initialize', { type: 'credit', price: price }, function (data) {
            if (data.status == 200) {
                window.location.href = data.url;
            } else {
