...
Class Aj {
	...
	public static function show() {
		...
		if ($class == NULL) {
			...
			if (is_callable(array(self::getInstance(), $class))) {
				$data = call_user_func(array(self::getInstance(), $class));
				if (!is_array($data)) {
					error_log("Error: \$data returned by $class is not an array.");
					$data = [];
				}
			} else {
				...
				if (file_exists($ajax_class_file)) {
					...
					$ob = new $class;
					if (is_callable(array($ob, $function))) {
						$data = (new $class)->$function();
						if (!is_array($data)) {
							error_log("Error: \$data returned by $class->$function() is not an array.");
							$data = [];
						}
					} else {
						$data['status'] = 405;
		...
		$status_message = self::getInstance()->get_http_response_text($data['status'] ?? 500);
	...
	public function test() {
		...
		error_log("Debug: \$data array contents: " . print_r($data, true));
		return $data;
	}
