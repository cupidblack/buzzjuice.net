...
class MysqliDb {
	...
	public function where($whereProp, $whereValue = 'DBNULL', $operator = '=', $cond = 'AND') {
		if ($whereValue === 'DBNULL') {
			// Skip conditions that are not meant to be added
			return $this;
		}
		if (is_array($whereValue) && ($key = key($whereValue)) != "0") {
			if ($key === null || $key === "" || empty($whereValue)) {
				//error_log("Warning: Invalid key or empty array in where() method. whereValue: " . print_r($whereValue, true));
				return $this; // Exit the method gracefully
			}
			$operator   = $key;
		...
		if ($operator == '0') {
			$operator = key($whereValue) ? key($whereValue) : '';
			if (empty($operator)) {
				error_log("⚠️ Operator fallback failed in where(). Prop: $whereProp, Value: " . print_r($whereValue, true));
				return $this;
			}
		}
		$this->_where[] = array(
	...
	protected function _buildCondition($operator, &$conditions) {
		...
		foreach ($conditions as $cond) {
			...
			// Validate varName and operator
			if (empty($varName) || (!isset($operator) || $operator === '')) {
				error_log("Warning: Invalid condition in _buildCondition(). varName: $varName, operator: $operator");
...
//	    error_log("Preparing query: " . $this->_query); // Log the query
