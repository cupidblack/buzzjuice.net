...
class SimpleImage {
  ...
  public function __construct($image = null) {
    ...
    if (is_string($image) && preg_match('/^data:(.*?);/', $image)) {
  ...
  public function fromFile($file) {
    // Ensure $file is a string and readable
    if (!is_string($file) || !is_readable($file)) {
        throw new \Exception("File not found or unreadable: $file", self::ERR_FILE_NOT_FOUND);
    }
    ...
    switch ($this->mimeType) {
        ...
        case 'image/webp':
            ...
            break;
        default:
            throw new \Exception("Unsupported image format: $file", self::ERR_UNSUPPORTED_FORMAT);
    }

    if (!$this->image) {
        throw new \Exception("Failed to load image: $file", self::ERR_INVALID_IMAGE);
    }

    // Convert palette images to true color
    imagepalettetotruecolor($this->image);

    // Load EXIF data for JPEG images
