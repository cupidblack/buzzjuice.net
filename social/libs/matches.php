...
class Matches{
    ...
    public function __construct(){
        ...
        $query = $db->objectBuilder()
                    ...
                    ->orderBy("id DESC")
                    ->get('users' ,30,array('id','username','avatar','country','first_name','last_name','birthday','language','relationship','height','body','smoke','ethnicity','pets'));
    ...
    public function Generateavatar(){
        ...
        if( !empty( $this->users ) ){
            ...
            foreach ($this->users as $key => $user ){
                ...
                $this->html .= '<img alt="'.$user->username.'" src="'.GetMedia($user->avatar).'"></div>';
                $i++;
            }
        }
        $this->html .= '</div>';
    ...
    public function show_user_details( $user, $hide = true ){
        ...
        if ($mediafiles) {
            foreach ($mediafiles as $key => $mediafile) {
                ...
                $mediafile->avatar = GetMedia(str_replace('_full.', '_avatar.', $mediafile->file), false);
            }
        ...
        if( isset( $mediafiles[0] ) ){
            $this->html .= '<div class="slide_prnt"><a href="'.$mediafiles[0]->full.'" data-fancybox="gallery"><div class="img_slide"><img alt="'.$full_name.'" src="'.$mediafiles[0]->avatar.'" class="responsive-img"></div></a></div>';
        }else{
            $this->html .= '<div class="slide_prnt"><div class="img_slide" data-fancybox="gallery"></div></div>';
        }
        if( isset( $mediafiles[1] ) ){
            $this->html .= '<div class="slide_prnt"><a href="'.$mediafiles[1]->full.'" data-fancybox="gallery"><div class="img_slide"><img alt="'.$full_name.'" src="'.$mediafiles[1]->avatar.'" class="responsive-img"></div></a></div>';
        }else{
            $this->html .= '<div class="slide_prnt"><div class="img_slide" data-fancybox="gallery"></div></div>';
        }
        if( isset( $mediafiles[2] ) ){
            $this->html .= '<div class="slide_prnt"><a href="'.$mediafiles[2]->full.'" data-fancybox="gallery"><div class="img_slide"><img alt="'.$full_name.'" src="'.$mediafiles[2]->avatar.'" class="responsive-img"></div></a></div>';
        }else{
            $this->html .= '<div class="slide_prnt"><div class="img_slide" data-fancybox="gallery"></div></div>';
        }
        if( isset( $mediafiles[3] ) ){
            $this->html .= '<div class="slide_prnt"><a href="'.$mediafiles[3]->full.'" data-fancybox="gallery"><div class="img_slide"><img alt="'.$full_name.'" src="'.$mediafiles[3]->avatar.'" class="responsive-img"></div></a></div>';
        }else{
            $this->html .= '<div class="slide_prnt"><div class="img_slide" data-fancybox="gallery"></div></div>';
        }
