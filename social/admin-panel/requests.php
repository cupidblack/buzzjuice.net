...
if ($f == "admin_setting" && (auth()->admin == '1' || CheckUserPermission(auth()->id, $p))) {
    ...
    if ($s == 'delete_photo') {
        if (!empty($_GET['photo_id'])) {
            ...
            $avatar_file = str_replace('_full.','_avatar.', $photo_file);
            $db->where('avatar',$avatar_file)->update('users',array( 'avatar' => $wo['config']['userDefaultAvatar'] ));
            $deleted = false;
            Wo_DeletePhoto($photo_id);
            if (DeleteFromToS3( $photo_file ) === true) {
                $deleted = true;
            }
            if (DeleteFromToS3( $avatar_file ) === true) {
                $deleted = true;
            }
            if ($deleted === true) {
                $data = array(
        ...
        try {
            ...
            if (!empty($buckets)) {
                if ($s3Client->doesBucketExist($wo['config']['bucket_name'])) {
                    $data['status'] = 200;
                    $array          = array(
                        '../streams/upload/photos/d-avatar.jpg'
                    );
                    foreach ($array as $key => $value) {
                        $s3Client->putObject([
    ...
    if ($s == 'test_ftp') {
        ...
        try {
            ...
            if ($login) {
                ...
                $array = array(
                    '../streams/upload/photos/fdsds.png'
                );
                foreach ($array as $key => $filename) {
                ...
                if ($bucket) {
                    ...
                    foreach ($array as $key => $value) {
                        ...
                        $storageObject = $bucket->upload($fileContent, array(
                            'name' => '../streams/upload/photos/d-avatar.jpg'
                        ));
                    }
                    $data['status'] = 200;
                } else {
        ...
        if ($users) {
            for ($i=0; $i < $count_users; $i++) {
                ...
                if ($avatar == 1) {
                    ...
                    $re_data['avatar'] = $wo['site_url'].'/upload/photos/users/'.rand(1,20).'.jpg';
                    $re_data['avatar'] = $users->ImportImageFromLogin($re_data['avatar'],1);
                    $imported_image = $re_data['avatar'];
    ...
    if ($s == 'add_new_blog_article' && Wo_CheckSession($hash_id) === true) {
        if (!empty($_POST['category']) && !empty($_POST['title']) && !empty($_POST['description'])) {
            ...
            $media_file = '../streams/upload/photos/d-blog.jpg';
    ...
    if ($s == 'update_qdw_pgb_config' && Wo_CheckSession($hash_id) === true) {
        $data = ['status' => 400, 'message' => 'Failed to update settings'];

        // Retrieve and sanitize the incoming data
        $qdw_payment = isset($_POST['qdw_payment']) ? Secure($_POST['qdw_payment']) : 'no';
        $qdw_store_url = isset($_POST['qdw_store_url']) ? Secure($_POST['qdw_store_url']) : '';
        $qdw_pro_package_id = isset($_POST['qdw_pro_package_id']) ? Secure($_POST['qdw_pro_package_id']) : '';
        $qdw_pro_package_id_2 = isset($_POST['qdw_pro_package_id_2']) ? Secure($_POST['qdw_pro_package_id_2']) : '';
        $qdw_pro_package_id_3 = isset($_POST['qdw_pro_package_id_3']) ? Secure($_POST['qdw_pro_package_id_3']) : '';
        $qdw_pro_package_id_4 = isset($_POST['qdw_pro_package_id_4']) ? Secure($_POST['qdw_pro_package_id_4']) : '';
        $qdw_credit_topup_id = isset($_POST['qdw_credit_topup_id']) ? Secure($_POST['qdw_credit_topup_id']) : '';
        $qdw_api_url = isset($_POST['qdw_api_url']) ? Secure($_POST['qdw_api_url']) : '';
        $qdw_api_key = isset($_POST['qdw_api_key']) ? Secure($_POST['qdw_api_key']) : '';
        $qdw_api_secret = isset($_POST['qdw_api_secret']) ? Secure($_POST['qdw_api_secret']) : '';
        $qdw_webhook_secret = isset($_POST['qdw_webhook_secret']) ? Secure($_POST['qdw_webhook_secret']) : '';

        // Validate input data
        if (!filter_var($qdw_store_url, FILTER_VALIDATE_URL)) {
            $data = ['status' => 400, 'message' => 'Invalid store URL'];
            echo json_encode($data);
            exit();
        }
        ...
        $update = $db->where('option_name', 'qdw_payment')->update('options', ['option_value' => $qdw_payment]) &&
                $db->where('option_name', 'qdw_store_url')->update('options', ['option_value' => $qdw_store_url]) &&
                $db->where('option_name', 'qdw_pro_package_id')->update('options', ['option_value' => $qdw_pro_package_id]) &&
                $db->where('option_name', 'qdw_pro_package_id_2')->update('options', ['option_value' => $qdw_pro_package_id_2]) &&
                $db->where('option_name', 'qdw_pro_package_id_3')->update('options', ['option_value' => $qdw_pro_package_id_3]) &&
                $db->where('option_name', 'qdw_pro_package_id_4')->update('options', ['option_value' => $qdw_pro_package_id_4]) &&
                $db->where('option_name', 'qdw_credit_topup_id')->update('options', ['option_value' => $qdw_credit_topup_id]) &&
                $db->where('option_name', 'qdw_api_url')->update('options', ['option_value' => $qdw_api_url]) &&
                $db->where('option_name', 'qdw_api_key')->update('options', ['option_value' => $qdw_api_key]) &&
                $db->where('option_name', 'qdw_api_secret')->update('options', ['option_value' => $qdw_api_secret]) &&
                $db->where('option_name', 'qdw_webhook_secret')->update('options', ['option_value' => $qdw_webhook_secret]);

        // Commit or rollback the transaction
        if ($update) {
            $db->commit();
            $data = ['status' => 200, 'message' => 'Settings updated successfully'];
        } else {
            $db->rollback();
            error_log("Failed to update QDW settings: " . $db->getLastError());
        }
