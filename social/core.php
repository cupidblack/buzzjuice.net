...
/**
 * QuickDate core.php — SSO-aware SessionStart() replacement
 *
 * Replaces the previous SessionStart implementation with a defensive, SSO-aware
 * bootstrap that:
 *  - Ensures a usable $_SESSION array.
 *  - Re-uses the WP-managed PHPSESSID when available (via cookie or buzz_sso payload).
 *  - Never destroys or regenerates WP PHPSESSID.
 *  - Hydrates SSO data from WordPress shadow files (sess_shadow_*) when present and readable.
 *  - Exposes the raw PHP-serialized shadow block in $_SESSION['buzz_sso_serialized'] for downstream normalization.
 *  - Falls back to validated buzz_sso cookie claims when shadow file is not available.
 *  - Logs structured diagnostics to BUZZ_DEBUG_LOG or qd_debug.log.
 *
 * Notes:
 *  - This file only replaces bz_bridge_log() and SessionStart() — leave the rest of core.php unchanged.
 */

/**
 * Bridge logging helper — writes JSON context to a debug log (non-blocking).
 * Uses BUZZ_DEBUG_LOG (WordPress side used same constant) if defined, else falls back.
 */
function bz_bridge_log($msg, $ctx = []) {
    $logfile = defined('BUZZ_DEBUG_LOG')
        ? BUZZ_DEBUG_LOG
        : (defined('BUZZ_SSO_BRIDGE_LOG') ? BUZZ_SSO_BRIDGE_LOG : dirname(__FILE__) . '/../shared/sso_sessions/qd_debug.log');
    ...
    if (!empty($ctx) && is_array($ctx)) $meta['ctx'] = $ctx;

    // Best-effort, non-fatal write
    @file_put_contents($logfile, '[' . $meta['ts'] . '] ' . $msg . ' | ' . json_encode($meta, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . PHP_EOL, FILE_APPEND | LOCK_EX);
}

/**
 * SSO-aware session bootstrap.
 *
 * - Accepts an optional $expected_user_id to initialize/verify session mapping without destructive operations.
 * - Idempotent: subsequent calls are no-ops.
 */
function SessionStart($expected_user_id = null) {
    ...
    $log = function($m, $ctx = []) { bz_bridge_log($m, $ctx); };

    // Ensure PHP uses php serialize handler for compatibility with WP exports.
    // Set before session_start() so session handling remains compatible where possible.
    if (ini_get('session.serialize_handler') !== 'php') {
        // Attempt to set; if not permitted this will silently fail, but we'll log.
        @ini_set('session.serialize_handler', 'php');
        if (ini_get('session.serialize_handler') !== 'php') {
            $log('Warning: session.serialize_handler is not "php" and could not be set', ['current'=>ini_get('session.serialize_handler')]);
        } else {
            $log('session.serialize_handler set to php');
        }
    }

    $sess_name = session_name();
    $wp_sid_candidate = null;

    // 1) Prefer explicit PHPSESSID cookie (canonical WP session id when present)
    if (!empty($_COOKIE[$sess_name])) {
        $wp_sid_candidate = preg_replace('/[^a-zA-Z0-9,_-]/', '', (string)$_COOKIE[$sess_name]);
        $log('Found PHPSESSID cookie candidate', ['cookie_name'=>$sess_name, 'candidate'=>$wp_sid_candidate]);
    }

    // 2) If no PHPSESSID cookie, try to extract session_id from buzz_sso cookie (validated)
    if (empty($wp_sid_candidate) && !empty($_COOKIE['buzz_sso'])) {
        $secret = defined('BUZZ_SSO_SECRET') ? BUZZ_SSO_SECRET : (getenv('BUZZ_SSO_SECRET') ?: null);
        if ($secret) {
            $parts = explode('.', (string)$_COOKIE['buzz_sso'], 2);
            if (count($parts) === 2) {
                ...
                if ($json !== false && $sig !== false) {
                    ...
                    if (hash_equals((string)$calc, (string)$sig)) {
                        $payload = @json_decode($json, true);
                        if (is_array($payload) && !empty($payload['session_id'])) {
                            $wp_sid_candidate = preg_replace('/[^a-zA-Z0-9,_-]/', '', (string)$payload['session_id']);
                            $log('Extracted session_id from buzz_sso cookie', ['session_id'=>$wp_sid_candidate]);
                        } else {
                            $log('buzz_sso cookie lacks session_id claim');
                        }
                    } else {
                        $log('buzz_sso cookie signature mismatch while extracting session_id');
                    }
                } else {
                    $log('buzz_sso cookie parts could not be base64url decoded');
                }
            } else {
                $log('buzz_sso cookie format unexpected while extracting session_id');
            }
        } else {
            $log('buzz_sso secret not configured; cannot extract session_id from cookie');
        }
    }

    // If a candidate exists and no session active, force session_id before session_start so QD reuses WP PHPSESSID.
    if (!empty($wp_sid_candidate) && session_status() === PHP_SESSION_NONE) {
        // Ensure candidate length sanity
        if (strlen($wp_sid_candidate) > 128) {
            $log('wp_sid_candidate length suspicious, ignoring', ['candidate_len'=>strlen($wp_sid_candidate)]);
            $wp_sid_candidate = null;
        } else {
            @session_id($wp_sid_candidate);
            // Start session
            @session_start();
            $log('Session started with candidate id', ['session_id'=>session_id(), 'wp_sid_candidate'=>$wp_sid_candidate]);
        }
    } else {
        // No candidate or session already active
        if (session_status() === PHP_SESSION_NONE) {
            @session_start();
            $log('Session started (no candidate)', ['session_id'=>session_id()]);
        } else {
            // Session already active — record mismatch if any
            $active_id = session_id();
            if (!empty($wp_sid_candidate) && $active_id !== $wp_sid_candidate) {
                $log('Session already active with different id; will NOT change PHPSESSID', ['active_id'=>$active_id, 'candidate'=>$wp_sid_candidate]);
                // Keep active id as canonical for this process.
            } else {
                $log('Session already active', ['session_id'=>$active_id]);
            }
        }
        // If session cookie appeared after start, capture it
        if (empty($wp_sid_candidate) && !empty($_COOKIE[$sess_name])) {
            $wp_sid_candidate = preg_replace('/[^a-zA-Z0-9,_-]/', '', (string)$_COOKIE[$sess_name]);
        }
    }

    // Guarantee $_SESSION is an array
    if (!isset($_SESSION) || !is_array($_SESSION)) $_SESSION = [];

    // Shadow helpers
    $shadow_dir = defined('BUZZ_SSO_SHADOW_PATH') ? BUZZ_SSO_SHADOW_PATH : (dirname(__FILE__) . '/../shared/sso_sessions');
    $shadow_file_for = function($derived_id) use ($shadow_dir) {
        return rtrim($shadow_dir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'sess_' . $derived_id;
    };
    $shadow_derived = function($wp_sid) {
        if (!$wp_sid) return null;
        return 'shadow_' . $wp_sid;
    };

    // If we have a candidate WP session id, attempt to read primary shadow file (PHP serialized)
    if (!empty($wp_sid_candidate)) {
        $derived = $shadow_derived($wp_sid_candidate);
        $path = $shadow_file_for($derived);

        // Prefer primary file; if missing try .ser then .json for diagnostics
        $tried = [];
        ...
        foreach ($candidates as $p) {
            if (is_file($p) && is_readable($p)) {
                ...
                if ($raw === false || $raw === '') {
                    $log('Shadow file exists but unreadable/empty', ['path'=>$p]);
                    continue;
                }

                // If this is a .json file but not serialized, keep it as JSON fallback but do not set buzz_sso_serialized to JSON.
                $is_json = (substr(trim($raw),0,1) === '{' || substr(trim($raw),0,1) === '[');
                if ($is_json && substr($p, -5) === '.json') {
                    // parse JSON and map allowed keys if present
                    $maybe_json = @json_decode($raw, true);
                    if (is_array($maybe_json)) {
                        $allow_keys = ['wp_user_id','wp_user_login','wp_user_email','wo_user_id','qd_user_id'];
                        $merged = [];
                        foreach ($allow_keys as $k) {
                            if (array_key_exists($k, $maybe_json) && (!isset($_SESSION[$k]) || $_SESSION[$k] === null)) {
                                $_SESSION[$k] = $maybe_json[$k];
                                $merged[] = $k;
                            }
                        }
                        // Do not assign buzz_sso_serialized for JSON; record the JSON debug presence
                        $log('Hydrated from shadow .json file (best-effort)', ['path'=>$p,'merged'=>$merged]);
                        break;
                    } else {
                        $log('Shadow .json present but JSON decode failed', ['path'=>$p]);
                        continue;
                    }
                }

                // Prefer PHP serialized payloads (primary .sess or .ser)
                // Assign raw serialized block to buzz_sso_serialized if not already set (downstream normalization expects it)
                if (empty($_SESSION['buzz_sso_serialized'])) {
                    $_SESSION['buzz_sso_serialized'] = $raw;
                }

                // Try unserialize (best-effort; do NOT trust classes or objects)
                ...
                if (is_array($maybe)) {
                    ...
                    $merged = [];
                    foreach ($allow_keys as $k) {
                        if (array_key_exists($k, $maybe) && (!isset($_SESSION[$k]) || $_SESSION[$k] === null)) {
                            $_SESSION[$k] = $maybe[$k];
                            $merged[] = $k;
                        }
                    }
                    $log('Hydrated from WP shadow serialized file', ['path'=>$p,'derived'=>$derived,'merged'=>$merged]);
                    break;
                } else {
                    $log('Shadow file present but unserialize did not return array', ['path'=>$p]);
                    // continue to next candidate (maybe .json exists)
                    continue;
                }
            } else {
                $tried[] = $p . ':missing';
        ...
        if (empty($tried)) {
            $log('No shadow files found for derived id', ['derived'=>$derived, 'expected_path'=>$path]);
        }
    } else {
        $log('No wp_sid_candidate available for shadow lookup');
    }

    // Fallback: if key WP session fields missing, hydrate selectively from validated buzz_sso cookie (do NOT set buzz_sso_serialized to JSON)
    if ((empty($_SESSION['wp_user_login']) || empty($_SESSION['wp_user_email'])) && !empty($_COOKIE['buzz_sso'])) {
        $secret = defined('BUZZ_SSO_SECRET') ? BUZZ_SSO_SECRET : (getenv('BUZZ_SSO_SECRET') ?: null);
        if ($secret) {
            $parts = explode('.', (string)$_COOKIE['buzz_sso'], 2);
            if (count($parts) === 2) {
                ...
                if ($json !== false && $sig !== false) {
                    ...
                    if (hash_equals((string)$calc, (string)$sig)) {
                        ...
                        if (is_array($claims)) {
                            ...
                            $merged = [];
                            foreach ($allow_keys as $k) {
                                if (isset($claims[$k]) && (!isset($_SESSION[$k]) || $_SESSION[$k] === null)) {
                                    $_SESSION[$k] = $claims[$k];
                                    $merged[] = $k;
                                }
                            }
                            if (!empty($claims['session_id']) && empty($_SESSION['wp_php_session_id'])) {
                                $_SESSION['wp_php_session_id'] = $claims['session_id'];
                            }
                            $_SESSION['qd_ready'] = !empty($_SESSION['qd_user_id']);
                            $log('Hydrated session from buzz_sso cookie (validated)', ['merged'=>$merged]);
                        } else {
                            $log('buzz_sso cookie JSON decode failed (pre-hydration)');
                        }
                    } else {
                        $log('buzz_sso cookie signature mismatch (pre-hydration)');
                    }
                } else {
                    $log('Could not base64url-decode buzz_sso cookie parts');
                }
            } else {
                $log('buzz_sso cookie format unexpected (pre-hydration)');
            }
        } else {
            $log('buzz_sso secret missing; skipping cookie hydration');
        }
    }

    // If an expected_user_id was provided, ensure session reflects that without destroying PHPSESSID.
    if ($expected_user_id !== null) {
        $expected_user_id = (int)$expected_user_id;
        $current_qd = isset($_SESSION['qd_user_id']) ? (int)$_SESSION['qd_user_id'] : null;
        if ($current_qd && $current_qd !== $expected_user_id) {
            $log('Expected qd_user_id mismatch; clearing SSO keys (non-destructive) and setting expected id', ['current'=>$current_qd,'expected'=>$expected_user_id]);
            $sso_keys = ['wp_user_id','wp_user_login','wp_user_email','wo_user_id','qd_user_id','qd_ready','expected_user_id','buzz_sso_last_sync','buzz_sso_serialized','wp_php_session_id','wp_session_name','wp_sso_login'];
            foreach ($sso_keys as $k) if (isset($_SESSION[$k])) unset($_SESSION[$k]);
            $_SESSION['qd_user_id'] = $expected_user_id;
            $_SESSION['expected_user_id'] = $expected_user_id;
        } elseif (empty($current_qd)) {
            $_SESSION['qd_user_id'] = $expected_user_id;
            $_SESSION['expected_user_id'] = $expected_user_id;
            $log('Initialized qd_user_id from expected_user_id', ['qd_user_id'=>$expected_user_id]);
        }
    }

    // Final safety: ensure $_SESSION array, produce keys list for debug
    if (!isset($_SESSION) || !is_array($_SESSION)) $_SESSION = [];
    $keys = array_keys($_SESSION);

    $log('SessionStart snapshot', [
        'session_name' => session_name(),
        'php_session_id' => session_id(),
        'wp_sid_candidate' => $wp_sid_candidate,
        ...
        'has_buzz_sso_serialized' => isset($_SESSION['buzz_sso_serialized']) ? true : false,
        'session_keys' => $keys
    ]);

    // IMPORTANT: do NOT regenerate or destroy the PHP session here — WordPress owns the lifecycle.
}

// Initialize session at bootstrap (idempotent)
SessionStart();
$config = new stdClass();
function LoadConfig() {
...
//    return $lang;
    ...
    if ( isset( $_COOKIE['JWT'] ) && !isset($_GET['language']) ) {
        ...
        if (isset($uid) && isset($uid['user_id'])) {
            $dafult_user_lang = $db->where('id', $uid['user_id'])->getOne('users');
        } else {
            error_log("Warning: 'uid' or 'user_id' is not set.");
            $dafult_user_lang = null; // Set a default value or handle the case where $uid['user_id'] is not available
        }
...
function auth() {
    ...
    // Return the logged-in user if already set
    if (!empty($loggedin_user) && !empty($loggedin_user->id)) {
        return $loggedin_user;
    }
    ...
    elseif (isset($_COOKIE['JWT']) && !empty($_COOKIE['JWT'])) {
        $token = $_COOKIE['JWT'];
    }
    // Check for access token in POST requests
    elseif (isset($_POST['access_token']) && !empty($_POST['access_token'])) {
        $token = $_POST['access_token'];
    }

    // If the user is logged in, validate the session or token
    if (IS_LOGGED === true) {
        if (!empty($loggedin_user) && !empty($loggedin_user->id)) {
            return $loggedin_user;
        }

        // Load user data from the session or token
        $user_id = GetUserFromSessionID($token);
        if ($user_id) {
            $loggedin_user = userData($user_id);
            return $loggedin_user;
        }
    ...
    // Handle endpoint requests for unauthenticated users
    if (IS_LOGGED === false && isEndPointRequest()) {
    ...
    //error_log("Error: User is not authenticated in auth().");
    return null;
}
...
/**
 * Core helpers (partial) - updated GetMedia() to handle absolute URLs and normalize paths
 *
 * Changes:
 * - Normalize backslashes to forward slashes early.
 * - If $media is already an absolute URL (http(s)://, //, data:, blob:) return it unchanged.
 * - Use safe concatenation (rtrim/ltrim) to avoid double slashes when building URLs.
 * - Preserve previous behavior for S3/Spaces/Wasabi/FTP/Cloud/Backblaze.
 * - Keep $config->s3_site_url set for callers that rely on it.
 */
...
function GetMedia($media, $allow_empty = true) {
    ...
    // Allow empty early if requested
    if ($allow_empty && $media === '') {
        return '';
    }

    // Normalize backslashes to forward slashes (helps Windows paths and some DB values)
    $media = str_replace('\\', '/', $media);

    // Setup default s3 url (kept for backward compatibility)
    $s3_site_url = 'https://test.s3.amazonaws.com';
    if (!empty($config->bucket_name) && !empty($config->amazone_s3) && (int)$config->amazone_s3 === 1) {
        $s3_site_url = 'https://' . rtrim($config->bucket_name, '/') . '.s3.amazonaws.com';
    }
    $config->s3_site_url = $s3_site_url;

    // If $media already looks like a full/absolute URL or data/blob URI, return as-is.
    // This handles cases where DB stores full URLs and prevents malformed concatenation.
    // Accepts: http://, https://, // (protocol-relative), data:, blob:
    if (
        filter_var($media, FILTER_VALIDATE_URL) ||
        strpos($media, '//') === 0 ||
        stripos($media, 'data:') === 0 ||
        stripos($media, 'blob:') === 0
    ) {
        return $media;
    }

    // Helper to join base + media reliably
    $join = function($base, $m) {
        return rtrim($base, '/') . '/' . ltrim($m, '/');
    };

    // If empty (and allow_empty == false) we will still build a URL from media (which may be empty string)
    // Legacy behavior: if S3 configured but credentials incomplete, fallback to site URI + media
    if (!empty($config->amazone_s3) && (int)$config->amazone_s3 === 1) {
        if (empty($config->amazone_s3_key) || empty($config->amazone_s3_s_key) || empty($config->region) || empty($config->bucket_name)) {
            return isset($config->uri) ? $join($config->uri, $media) : $media;
        }
        return $join($config->s3_site_url, $media);
    }
    elseif (!empty($config->spaces_key) && !empty($config->spaces) && (int)$config->spaces == 1) {
        $base = 'https://' . rtrim($config->space_name, '.') . '.' . rtrim($config->space_region, '.') . '.digitaloceanspaces.com';
        return $join($base, $media);
    }
    elseif (!empty($config->wasabi_access_key) && !empty($config->wasabi_storage) && (int)$config->wasabi_storage == 1) {
        $config->wasabi_site_url = "https://s3." . $config->wasabi_bucket_region . ".wasabisys.com";
        if (!empty($config->wasabi_bucket_name)) {
            $config->wasabi_site_url = "https://s3." . $config->wasabi_bucket_region . ".wasabisys.com/" . rtrim($config->wasabi_bucket_name, '/');
            return $join($config->wasabi_site_url, $media);
        }
    }
    elseif (!empty($config->ftp_host) && !empty($config->ftp_upload) && (int)$config->ftp_upload == 1) {
        $endpoint = isset($config->ftp_endpoint) ? $config->ftp_endpoint : $config->ftp_host;
        // Ensure ftp endpoint includes protocol
        if (!preg_match('#^https?://#i', $endpoint)) {
            $endpoint = 'http://' . $endpoint;
        }
        return $join($endpoint, $media);
    }
    elseif (!empty($config->cloud_bucket_name) && !empty($config->cloud_upload) && (int)$config->cloud_upload == 1) {
        $base = 'https://storage.googleapis.com/' . rtrim($config->cloud_bucket_name, '/');
        return $join($base, $media);
    }
    elseif (!empty($config->backblaze_storage) && (int)$config->backblaze_storage == 1) {
        if (!empty($config->backblaze_endpoint) && filter_var($config->backblaze_endpoint, FILTER_VALIDATE_URL)) {
            return $join($config->backblaze_endpoint, $media);
        }
        return $join('https://' . rtrim($config->backblaze_bucket_name, '.') . '.s3.' . $config->backblaze_bucket_region . '.backblazeb2.com', $media);
    }

    // Default local uri
    if (!empty($config->uri)) {
        return $join($config->uri, $media);
    }

    // Last resort: return media as-is
    return $media;
}
...
function Secure($string, $br = true, $strip = 0) {
    ...
    $string = trim($string ?? ''); // Handle null values
...
function userProfile($username, $cols = array(),$only_token = false){
    ...
    if ($db->count > 0) {
        ...
        if (isEndPointRequest()) {
            $profile->avatar = GetMedia($user->avatar, false);
        }else{
            $profile->avatar = new stdClass();
            $profile->avatar->full = GetMedia(str_replace('_avatar.', '_full.', $user->avatar), false);
            $profile->avatar->avatar = GetMedia($user->avatar, false);
        }
        ...
        if ($mediafiles) {
            ...
            foreach ($mediafiles as $mediafile) {
                $mf = array(
                    ...
                    'avatar' => GetMedia(str_replace('_full.', '_avatar.', $mediafile['file']), false),
                    'is_private' => $mediafile['is_private'],
                    'private_file_full' => GetMedia( $mediafile['private_file'], false),
                    'private_file_avatar' => GetMedia(str_replace('_full.', '_avatar.', $mediafile['private_file']), false),
        ...
        if(isEndPointRequest()){
            ...
            if(!empty($user->id) && !is_avatar_approved($user->id, $user->avatar)) {
                $profile->avatar->full = GetMedia($config->userDefaultAvatar, false);
                $profile->avatar->avatar = GetMedia($config->userDefaultAvatar, false);
            }
        }
        return $profile;
    }else{
        if(!empty($user) && !empty($user->id) && !empty($user->avatar) && !is_avatar_approved($user->id, $user->avatar)) {
            $profile->avatar->full = GetMedia($config->userDefaultAvatar, false);
            $profile->avatar->avatar = GetMedia($config->userDefaultAvatar, false);
        }
        return $profile;
...
function GetUserFromSessionID($session_id, $platform = 'web') {
    ...
    if (!$query || mysqli_num_rows($query) === 0) {
        //error_log("Error: No session found for session_id: {$session_id}");
        return false;
    }
    ...
    if (empty($fetched_data) || !isset($fetched_data['user_id'])) {
        error_log("Error: Invalid session data for session_id: {$session_id}");
        return false;
    }
    ...
    if (empty($fetched_data['platform_details']) && $fetched_data['platform'] == 'web') {
        $ua = serialize(GetBrowser());
        $db->where('id', $fetched_data['id'])->update('sessions', array('platform_details' => $ua));
    }
...
function IsGotCredit()
{
    ...
    // Check if the credit earning system is enabled
    if ($config->credit_earn_system == 0) {
        return false;
    }

    // Define maximum days and daily credit amount
    $max_days = (int)$config->credit_earn_max_days;
    $day_amount = (int)$config->credit_earn_day_amount;
    ...
    // Query the database for today's credits
    ...
    // Check if there are any credits for today
    if (!empty($count)) {
        foreach ($count as $key => $value) {
            if ($value->added == 1) {
                return false; // Credit already added for today
            }
        }

        // Check if the user has reached the maximum days
        if ($max_days <= count($count)) {
...
function getMessageContainer($message){
    ...
    if( $message->type == 'received' ){
        $class = "r";
        $avatar = '     <div class="m_avatar"><img src="'. GetMedia($message->from_avatar) .'" alt="'. $message->to_name .'" title="'. $message->to_name .'"></div>' . "\n";
...
function logout($redirect = true){
    ...
    // Instead of redirecting straight to home, redirect to central SSO logout proxy
    // The shared proxy will call WP logout server-side and then redirect to WoWonder chain
    $secret = getenv('BUZZ_SSO_SECRET') ?: (defined('BUZZ_SSO_SECRET') ? BUZZ_SSO_SECRET : '');
    if ($secret) {
        $proxy = 'https://buzzjuice.net/shared/sso-logout.php?sso_secret=' . rawurlencode($secret) . '&from=quickdate';
        header('Location: ' . $proxy);
        exit();
    }
    
    // Fallback (if secret missing)
    header("Location: /social/logout/?social=home");
    exit();
...
function generate_chat_messages_convirsation($user_id,$to,$offset = 0,$show_unreadline = true,$lastmsg = 0,$prev=false){
    ...
    $chat_messages = $db->arrayBuilder()->get('messages m',$limit,array('m.id','u.username as from_name','u.avatar as from_avatar','u1.username as to_name','u1.avatar as to_avatar','m.`from`','m.`to`','m.text','m.media','m.from_delete','m.to_delete','s.file as sticker','m.created_at','m.seen'));
...
function sendOneSignalPush($data = array()){
    ...
    $notify['userdata'] = $db->where('id', $data['data']['notifier_id'])->getOne('users',array('id','username','first_name','last_name','avatar'));
    ...
    $notify['userdata'] = $db->where('id', $data['data']['notifier_id'])->getOne('users',array('id','username','first_name','last_name','avatar'));
    $notify['userdata']['avatar'] = GetMedia($notify['userdata']['avatar']);
    $fields['data'] = $notify;
    //}
...
//BlueCrownR&D - Updated membership time
function DeleteExpiredProMemebership() {
    global $conn,$db;
    $week_duration   = 2629743; // updated to 1 month
    $month_duration    = 7889229; // updated to 3 months
    $year_duration = 15778458; // updated to 6 months
    $life_duration    = 31556926; // updated to 12 Months
...
function ShareFile($data = array(), $type = 0, $crop = true,$fldr=false) {
    ...
    if (!file_exists($path.'../streams/upload/photos/' . date('Y'))) {
        @mkdir($path.'../streams/upload/photos/' . date('Y'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/photos/' . date('Y') . '/' . date('m'))) {
        @mkdir($path.'../streams/upload/photos/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/videos/' . date('Y'))) {
        @mkdir($path.'../streams/upload/videos/' . date('Y'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/videos/' . date('Y') . '/' . date('m'))) {
        @mkdir($path.'../streams/upload/videos/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/sounds/' . date('Y'))) {
        @mkdir($path.'../streams/upload/sounds/' . date('Y'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/sounds/' . date('Y') . '/' . date('m'))) {
        @mkdir($path.'../streams/upload/sounds/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/gifts/' . date('Y'))) {
        @mkdir($path.'../streams/upload/gifts/' . date('Y'), 0777, true);
    }
    if (!file_exists($path.'../streams/upload/gifts/' . date('Y') . '/' . date('m'))) {
        @mkdir($path.'../streams/upload/gifts/' . date('Y') . '/' . date('m'), 0777, true);
    }
    ...
    if( $fldr == 'gifts' || $fldr == 'stickers' ){
        $filename    = '../streams/upload/' . "{$fldr}" . DIRECTORY_SEPARATOR . $fn;
    }
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        ...
        $last_data             = array();
        if( $fldr == 'gifts' || $fldr == 'stickers' ) {
            $last_data['filename'] = '../streams/upload/'.$fldr.'/'.$fn;
        }else{
            if( $fldr !== false && $fldr == 'blogs' ){
                $last_data['filename'] = '../streams/upload/photos/'. date('Y') . '/' . date('m') . '/' . $fn;
            }else{
...
function GetUserTotalLikes($userid) {
    global $conn;
    $data = array();
    $data['LikeCount'] = 0; // Default value
    $query_one = "SELECT COUNT(`id`) as LikeCount FROM `likes` WHERE `like_userid` = " . (int)$userid . " AND `created_at` >= (DATE_SUB(CURDATE(), INTERVAL 5 MINUTE));";
    $sql = mysqli_query($conn, $query_one);
    if ($sql) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            if (isset($fetched_data['LikeCount'])) {
                $data['LikeCount'] = (int)$fetched_data['LikeCount'];
            }
        }
    }
    return $data['LikeCount'];
...
function GetUserTotalMatches($userid) {
    global $conn;
    $data = array();
    $data['MatcheCount'] = 0; // Default value
    $query_one = "SELECT COUNT(`id`) as MatcheCount FROM `notifications` WHERE `recipient_id` = " . (int)$userid . " AND `type` = 'got_new_match' AND `created_at` < NOW() AND `created_at` > UNIX_TIMESTAMP(DATE_ADD(NOW(), INTERVAL -50 MINUTE));";
    $sql = mysqli_query($conn, $query_one);
    if ($sql) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            if (isset($fetched_data['MatcheCount'])) {
                $data['MatcheCount'] = (int)$fetched_data['MatcheCount'];
            }
        }
    }
    return $data['MatcheCount'];
...
//    }
    ...
    $query          = mysqli_query($conn, "SELECT COUNT(`id`) FROM `blocks` WHERE (`user_id` = {$logged_user_id} AND `block_userid` = {$user_id}) OR (`user_id` = {$user_id} AND `block_userid` = {$logged_user_id})");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}
//done
...
function is_avatar_approved($user_id, $avatar){
    ...
    $avatar = str_replace( '_avatar.' , '_full.', $avatar);
...
function ImportFileFromUrl($url,$type,$upload_storage = true)
{
    ...
    if (!empty($file)) {
        if ($type == 'VIDEO') {
            ...
            $org_file = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '_video.mp4';
            if ($upload_storage) {
                if (is_file($org_file)) {
        ...
        elseif ($type == 'IMAGE') {
            ...
            $org_file = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.jpg';
...
function getStatus($config = array()) {
    ...
    $dirs = array_filter(glob('../streams/upload/*'), 'is_dir');
