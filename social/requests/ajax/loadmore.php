        ...
        if ($error == '') {
            ...
            if (file_exists($template)) {
                ...
                foreach ($random_users as $random_user) {
                    ob_start();
                    if(!is_avatar_approved($random_user->id, $random_user->avatar)) {
                        $random_user->avatar = $config->userDefaultAvatar;
                    }
            ...
            if ($with_html) {
                if (file_exists($template)) {
                    ...
                    foreach ($liked_users as $row) {
                        ob_start();
                        if(!is_avatar_approved($row->id, $row->avatar)) {
                            $row->avatar = $config->userDefaultAvatar;
                        }
                        include $template;
                        $html .= ob_get_contents();
                        ob_end_clean();
        ...
        if ($error == '') {
            ...
            foreach ($blockes as $key => $user) {
                $data[$key] = $db->objectBuilder()
                                        ...
                                        ->getOne('users',array('id','username','first_name','last_name','avatar'));
        ...
        if ($error == '') {
            ...
            if (file_exists($template)) {
                ...
                foreach ($match_users_array as $matche) {
                    ...
                    ob_start();
                    if($value['src'] !== 'Fake'){
                        if(!is_avatar_approved($matche->id, $matche->avatar)) {
                            $matche->avatar = $config->userDefaultAvatar;
                        }
                    }
                    include $template;
                    $matche_first = false;
