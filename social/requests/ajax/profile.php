...
Class Profile extends Aj {
    ...
    public function unprivate_avatar(){
        ...
        if ($error == '') {
            ...
            if ($exist[ 'file' ] == self::ActiveUser()->avatar->full) {
                return array(
                    'status' => 400,
            ...
            if ($id[0]['id'] > 0) {
                ...
                $avatar_file = str_replace('_full.', '_avatar.', $id[0]['private_file']);
                if($id[0]['is_video'] == 0) {
                    DeleteFromToS3($id[0]['private_file']);
                    DeleteFromToS3($avatar_file);
                }
            }
            return array(
                'status' => 200,
    ...
    public function private_avatar() {
        global $db, $_UPLOAD, $_DS,$config;
        if (self::ActiveUser() == NULL) {
            return array(
                'status' => 403,
        ...
        if ($error == '') {
            ...
            if ($id[0]['id'] > 0) {
                ...
                $avatar_file = str_replace('_full.', '_avatar.', $id[0]['file']);
                ...
                $safe_dest = '../streams/upload/photos/private/' . date('Y') . '/' . date('m') . '/'  . $key . '_private_full.'.$file_extension;
                $safe_dest_thumb = '../streams/upload/photos/private/' . date('Y') . '/' . date('m') . '/' . $key . '_private_avatar.'.$file_extension;
    ...
    public function delete_avatar() {
        ...
        if ($error == '') {
            ...
            if ($id > 0) {
                ...
                $avatar_file = str_replace('_full.', '_avatar.', $exist[ 'file' ]);
                DeleteFromToS3($exist[ 'file' ]);
                DeleteFromToS3($avatar_file);

                $private_file = $db->where('id', $id)->getValue('mediafiles', 'private_file');
                if($private_file !== ''){
                    $avatar_file = str_replace('_full.', '_avatar.', $private_file);
                    DeleteFromToS3($avatar_file);
                ...
                if ($exist[ 'file' ] == str_replace($site_url . "/","",self::ActiveUser()->avatar->full)) {
                    $db->where('id', self::ActiveUser()->id)->update('users',array('avatar' => self::Config()->userDefaultAvatar));
                }
            }
            return array(
                'status' => 200,
    ...
    public function delete_avatar_admin() {
        global $db, $_UPLOAD, $_DS;
        ...
        if ($error == '') {
            ...
            if ($id > 0) {
                ...
                $db->where('id', $id)->delete('mediafiles');
                if ($_POST[ 'url' ] == $userData->avatar->full) {
                    $last_img = $db->where('user_id', $uid)->orderBy('id','DESC')->getValue('mediafiles', 'file');
                    if($last_img){
                        $db->where('id', $uid)->update('users',array( 'avatar' => str_replace('_full.', '_avatar.', $last_img) ));
                    }else{
                        $db->where('id', $uid)->update('users',array( 'avatar' => self::Config()->userDefaultAvatar ));
                    }
                }
        ...
        if($error === false){
            ...
            if (self::Config()->ffmpeg_sys == '1'){
                ...
                if (move_uploaded_file($_FILES['video_thumbnail']['tmp_name'], $filename)) {
                    $org_file = $_UPLOAD . 'photos' . $_DS . date('Y') . $_DS . date('m') . $_DS . $key . '_full.' . $ext;
                    $org_file2 = $_UPLOAD . 'photos' . $_DS . date('Y') . $_DS . date('m') . $_DS . $key . '.' . $ext;
                    if (is_file($org_file)) {
                    ...
                    $media['is_private'] = (int)Secure($_POST['privacy']);
                    $media['is_confirmed'] = '1';
                    $media['file'] = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext;
                    $media['private_file'] = ((int)Secure($_POST['privacy']) === 1) ? '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext : '';
                    $saved = $db->where('id', (int)Secure($_POST['media_id']))->where('user_id', self::ActiveUser()->id)->update('mediafiles', $media);
                    if ($saved) {
                        return array(
                            'status' => 200
    ...
    public function upload_video() {
        ...
        $img_dir = '../streams/upload/photos/' . date('Y') . '/' . date('m');
        $img_path = '../streams/upload/photos/' . date('Y') . '/' . date('m');
        ...
        if (self::Config()->ffmpeg_sys == '1' && !empty($_FILES['video']) && file_exists($_FILES['video']['tmp_name'])){
            ...
            if (move_uploaded_file($_FILES['video']['tmp_name'], $filename)) {
                $org_file = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext;
                if (is_file($org_file)) {
                    // $upload_s3 = UploadToS3($org_file, array(
                    //     'amazon' => 0
                    // ));
                ...
                if((int)self::Config()->max_video_duration > 0) {
                    ...
                    $video = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '.final.mp4';
                    
                    //@unlink($org_file);
                //}else{
                //    $video = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext;
                //}
            ...
            if (move_uploaded_file($_FILES['video']['tmp_name'], $filename)) {
                $org_file = $_UPLOAD . 'videos' . $_DS . date('Y') . $_DS . date('m') . $_DS . $key . '.' . $ext;
                if (is_file($org_file)) {
                    $upload_s3 = UploadToS3($org_file, array(
                        'amazon' => 0
                    ));
                ...
                $media['user_id'] = self::ActiveUser()->id;
                $media['is_video'] = '1';
                $media['video_file'] = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext;
                $media['created_at'] = date('Y-m-d H:i:s');
                $saved = $db->insert('mediafiles', $media);
                if ($saved) {
                    $file = '../streams/upload/videos/' . date('Y') . '/' . date('m') . '/' . $key . '.' . $ext;
                    return array(
                        'status' => 200,
                        'video_file' => $file,
                        'media_id' => $saved
    ...
    public function upload_verification_photo(){
        ...
        foreach ($_FILES as $file) {
            ...
            if (move_uploaded_file($file[ 'tmp_name' ], $filename)) {
                ...
                $media[ 'photo' ]       = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
                ...
                if ($saved) {
                    ...
                    $files[]                  = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
                }
            } else {
                $error = true;
            }
    ...
    public function upload_verification_passport(){
        ...
        foreach ($_FILES as $file) {
            ...
            if (move_uploaded_file($file[ 'tmp_name' ], $filename)) {
                ...
                $media[ 'user_name' ] = self::ActiveUser()->username;
                $media[ 'passport' ]       = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
                if( !empty($id[0]) ){
                    $saved                 = $db->where('id',$id[0]['id'])->update('verification_requests', $media);
                    if($id[0]['photo'] !== '' ){
                        $class = "";
    ...
    public function upload_avatar() {
        ...
        foreach ($_FILES as $file) {
            ...
            if (move_uploaded_file($file[ 'tmp_name' ], $filename)) {
                ...
                $thumbfile = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.' . $ext;
                $org_file  = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
                $oreginal  = new ImageThumbnail($filename);
                $oreginal->setResize(false);
                $oreginal->save($org_file);
                ...
                $media                 = array();
                $media[ 'user_id' ]    = self::ActiveUser()->id;
                $media[ 'file' ]       = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
                $media[ 'created_at' ] = date('Y-m-d H:i:s');
                ...
                if ($saved) {
                    $_SESSION[ 'userEdited' ] = true;
                    $files[]                  = '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.' . $ext;
                }
            } else {
                $error = true;
            }
        ...
        if ($error) {
            ...
            error_log("File upload response: " . json_encode(array(
                'status' => 200,
                'files' => $files,
            )));
    ...
    public function set_avatar() {
        ...
        if (empty($_GET[ 'id' ])) {
            ...
            $id = Secure($_GET[ 'id' ]);
            if ($id != self::ActiveUser()->avatar->avatar) {
...
//            }
    ...
    public function set_snapshotdata(){
        ...
        if (move_uploaded_file($_FILES['thumb'][ 'tmp_name' ], $filename)) {
            ...
            $data['snapshot'] = '../streams/upload/snapshots/' . date('Y') . '/' . date('m') . '/' . $key . '.'.$ext;
        ...
        //     if (is_file($fileName)) {
        //         $safe_dir = '../streams/upload/snapshots/' . date('Y') . '/' . date('m') . '/' . $key . '.jpg';
        ...
        //         $data['verified'] = "0";
        //     }
        //     $data['snapshot'] = '../streams/upload/snapshots/' . date('Y') . '/' . date('m') . '/' . $key . '.jpg';
    ...
    public function upload_receipt() {
        ...
        foreach ($_FILES as $file) {
            ...
            if (move_uploaded_file($file[ 'tmp_name' ], $filename)) {
                ...
                $org_file  = $_UPLOAD . 'photos' . $_DS . date('Y') . $_DS . date('m') . $_DS . $key . '_avatar.' . $ext;
                $oreginal  = new ImageThumbnail($filename);
                $oreginal->setResize(false);
                $oreginal->save($org_file);
                @unlink($filename);
                ...
                $info[ 'receipt_file' ]= '../streams/upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_avatar.' . $ext;
    ...
    public function my_info()
    {
        ...
        if (!empty($_POST['mediafiles']) || !empty($_POST['my_information']) || !empty($_POST['friends']) || !empty($_POST['liked_users']) || !empty($_POST['disliked_users'])) {
            ...
            if (!file_exists('../streams/upload/files/' . date('Y'))) {
                @mkdir('../streams/upload/files/' . date('Y'), 0777, true);
            }
            if (!file_exists('../streams/upload/files/' . date('Y') . '/' . date('m'))) {
                @mkdir('../streams/upload/files/' . date('Y') . '/' . date('m'), 0777, true);
            }
            $folder   = 'files';
            $fileType = 'file';
